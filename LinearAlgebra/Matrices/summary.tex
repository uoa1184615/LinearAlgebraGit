...
\itemme In modelling the age structure of populations, the so-called \idx{Leslie matrix} encodes the birth, ageing, and death (of females) in the population to empower predictions of the future population.
...
\begin{itemize}
        ...
        \item  $m\times n$ \idx{diagonal matrix}~$S$ is zero except for unique non-negative \idx{diagonal elements} called \bfidx{singular value}s
...
\itemme Recall that in the context of a system of linear equations \(A\xv=\bv\) with \(m\times n\) matrix~\(A\), for every \(\bv\in\RR^m\) \autoref{pro:appsol} finds the smallest solution~\(\xv\in\RR^m\)\ (\autoref{thm:smallsoln}) to the closest \idx{consistent} system \(A\xv=\tilde\bv\) (\autoref{thm:appsol}).
