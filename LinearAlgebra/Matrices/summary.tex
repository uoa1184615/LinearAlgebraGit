...
\begin{itemize}
\item A \bfidx{matrix} is a rectangular array of real numbers, written inside \bfidx{brackets}~\(\begin{bmatrix} \cdots  \end{bmatrix}\)---create in \script\ with \verb|[...;...;...]|.

\item The \bfidx{size} of a matrix is written \(m\times n\) where \(m\)~is the number of rows and \(n\)~is the number of columns---compute with \verb|size(A)| for matrix~\verb|A|.
...
\item \(O_{m\times n}\)~denotes the \(m\times n\) zero matrix, \(O_n\)~denotes the square zero matrix of size \(n\times n\)---compute with \verb|zeros(m,n)| and \verb|zeros(n)|, respectively---whereas \(O\)~denotes a zero matrix whose size is apparent from the context.

\item The \bfidx{identity matrix}~\(I_n\) denotes a \(n\times n\) square matrix which has zero entries except for the diagonal from the top-left to the bottom-right which are all ones---compute with~\verb|eye(n)|.
...
\item For \(m\times n\) matrix~\(A\) and vector \(\xv\) in~\(\RR^n\), the \bfidx{matrix-vector product}~\(A\xv\)\,---compute by \verb|A*x|---is the following vector in~\(\RR^m\),
\begin{equation*}
A\xv:=
...
\end{bmatrix}.
\end{equation*}
Multiplication of a vector by a square matrix transforms the vector into another in the same space.

\item For \(m\times n\) matrix~\(A\), and \(n\times p\) matrix~\(B\), the \bfidx{matrix product} \(C=AB\)\,---compute by \verb|C=A*B|---is the \(m\times p\) matrix whose \((i,j)\)th~entry is
\begin{equation*}
...
\item Matrix addition and scalar multiplication satisfy familiar properties (\autoref{thm:pasm}): 
...
\item Matrix multiplication also satisfies familiar properties (\autoref{thm:pmm}):
...
\item The \bfidx{transpose} of an \(m\times n\) matrix~\(A\) is the \(n\times m\) matrix \(B=\tr A\) with entries \(b_{ij}=a_{ji}\) (\autoref{def:mattran})---compute by~\verb|A'|.

A matrix~\(A\) is a \bfidx{symmetric matrix} if \(\tr A=A\) (\autoref{def:matsym}).
A symmetric matrix must be a square matrix

\item The matrix transpose satisfies (\autoref{thm:pot}):
\(\tr{(\tr A)}=A\);
\(\tr{(A\pm B)}=\tr A\pm \tr B\);
\(\tr{(cA)}=c(\tr A)\);
\(\tr{(AB)}=\tr B\tr A\);
...
\item An \bfidx{inverse} of a square matrix~\(A\) is a matrix~\(B\) such that both \(AB=I\) and \(BA=I\) (\autoref{def:invertible}).
If such a matrix~\(B\) exists, then matrix~\(A\) is called \bfidx{invertible}.

\item If \(A\) is an {invertible} matrix, then its {inverse} is unique, and denoted by~\(A^{-1}\) (\autoref{thm:uninv}).

\item For every \(2\times2\) matrix \(A=\begin{bmatrix} a&b\\c&d \end{bmatrix}\), matrix~\(A\) is {invertible} if and only if the \bfidx{determinant} \(ad-bc\neq0\) (\autoref{thm:2x2det}), in which case
\begin{equation*}
A^{-1}=\frac1{ad-bc}\begin{bmatrix} d&-b\\-c&a \end{bmatrix}.
\end{equation*}

\item If matrix~\(A\) is {invertible}, then  \(A\xv=\bv\) has the {unique solution} \(\xv=A^{-1}\bv\) for every~\(\bv\) (\autoref{thm:invuniqsol}).

\item For all invertible matrices~\(A\) and~\(B\), the inverse has the properties (\autoref{thm:invprop}):
\begin{itemize}
\item  matrix \(A^{-1}\) is invertible and \((A^{-1})^{-1}=A\)\,;
\item if scalar \(c\neq0\)\,, then matrix~\(cA\) is invertible and \((cA)^{-1}=\frac1cA^{-1}\);
\item matrix \(AB\) is invertible and \((AB)^{-1}=B^{-1}A^{-1}\);
...
\item The \bfidx{diagonal entries} of an \(m\times n\) matrix~\(A\) are  \(a_{11},a_{22},\ldots,a_{pp}\) where \(p=\min(m,n)\).
...
\item In \script\ (but not in algebra), \verb|diag| also does the opposite: for an \(m\times n\) matrix~\(A\) such that both \(m,n\geq2\)\,,  \verb|diag(A)| returns the (column) vector \((a_{11},a_{22},\ldots,a_{pp})\) of diagonal entries where the result vector length \(p=\min(m,n)\).

\item The dot operators \index{./@\texttt{./}}\verb|./| and~\index{.*@\texttt{.*}}\verb|.*| perform element-by-element division and multiplication of two matrices\slash vectors of the same size.
...
\item A set of non-zero vectors \(\{\hlist\qv k\}\) is called an \bfidx{orthogonal set} if all pairs of distinct vectors in the set are {orthogonal}: that is, \(\qv_i\cdot\qv_j=0\) whenever \(i\neq j\) for \(i,j=1,2,\ldots,k\) (\autoref{def:orthoset}).
A set of vectors is called an \bfidx{orthonormal set} if it is an {orthogonal set} of {unit vector}s.

\item A square matrix~\(Q\) is called an \bfidx{orthogonal matrix} if \(\tr QQ=I\) (\autoref{def:orthog}).
Multiplication by an orthogonal matrix is called a \bfidx{rotation and/or reflection}.

\item For every square matrix~\(Q\),  the following statements are equivalent (\autoref{thm:orthog}):
\begin{itemize}
...
\item Every $m\times n$ real matrix~$A$ can be factored into a product of three matrices \(A=\usv\) (\autoref{thm:svd}),
...
\item \autoref{pro:gensol} derives a general solution of the system $A\xv=\bv$ using an \svd.
\begin{enumerate}
\item Obtain an \svd\ factorisation \(A=\usv\).
\item Solve \(U\zv=\bv\) by $\zv=\tr U\bv$ (unique given~\(U\)).

\item To solve \(S\yv=\zv\), identify the non-zero and the zero \idx{singular value}s: suppose \(\sigma_1\geq\sigma_2\geq\cdots\geq\sigma_r>0\) and \(\sigma_{r+1}=\cdots=\sigma_{\min(m,n)}=0\):
\begin{itemize}
\item if $z_i\neq0$ for any \(i=r+1,\ldots,m\)\,, then there is {no solution} (the equations are {inconsistent});
...
\item The \bfidx{condition number} of a matrix~\(A\) is the the ratio of the largest to smallest of its {singular value}s: \(\cond A:=\sigma_1/\sigma_{\mn}\) (\autoref{def:condnum}); 
if \(\sigma_{\mn}=0\)\,, then \(\cond A:=\infty\)\,; 
also, \(\cond O_{m\times n}:=\infty\)\,.

\item The \bfidx{rank} of a matrix~$A$ is the number of \emph{nonzero} {singular value}s in an~\svd, \(A=\usv\) (\autoref{def:rank}).

\item For every matrix~\(A\), let an \svd\ of~\(A\) be~\(\usv\), then the transpose~\(\tr A\) has an \svd\ of \(V(\tr S)\tr U\) (\autoref{thm:ranktr}). 
...
\item Consider solving \(A\xv=\bv\) for \(n\times n\) matrix~\(A\) with full \(\rank A=n\)\,.  
...
\item A \bfidx{subspace}~\WW\ of~\(\RR^n\),  is a set of vectors, including \(\ov\in\WW\), such that \WW\ is \bfidx{closed} under addition and scalar multiplication: that is, for all \(c\in\RR\) and \(\uv,\vv\in\WW\), then both \(\uv+\vv\in\WW\) and \(c\uv\in\WW\) (\autoref{def:subspace}).
...
\item For any $m\times n$ matrix~$A$, define~$\Null(A)$ to be the set of all solutions~$\xv$ to the {homogeneous} system $A\xv=\ov$\,. 
The set~\(\Null(A)\) is a {subspace} of~$\RR^n$ called the \bfidx{nullspace} of~$A$ (\autoref{thm:homosubsp}).

\item An \bfidx{orthonormal basis} for a {subspace}~\WW\ of~\(\RR^n\) is an {orthonormal set} of vectors that span~\WW\ (\autoref{def:orthobasis}).

\item  \autoref{pro:ospan} finds an {orthonormal basis} for the subspace \(\Span\{\hlist\av n\}\), where $\{\hlist\av n\}$ is a set of $n$~vectors in~\(\RR^m\). 
\begin{enumerate}
...
\item For every \WW\ be a \idx{subspace} of~\(\RR^n\), the number of vectors in an \idx{orthonormal basis} for~\WW\ is called the \bfidx{dimension} of~\WW, denoted~\(\dim\WW\) (\autoref{def:dim}).
By convention, \(\dim\{\ov\}=0\)\,.

\item The \idx{row space} and \idx{column space} of a matrix~\(A\) have the same \idx{dimension} (\autoref{thm:rowcolD}).
...
\item For every \(m\times n\) matrix~\(A\),  \(\rank A+\nullity A=n\)\,, the number of columns of~\(A\) (\autoref{thm:rank}).

\item For every \(n\times n\) \idx{square matrix}~\(A\), and extending \autoref{thm:ftim1}, the following statements are equivalent (\autoref{thm:ftim2}):
\begin{itemize}
...
\item \autoref{pro:appsol} computes the `\idx{least square}' approximate solution(s) of \idx{inconsistent} equations $A\xv=\bv$\,:
    \begin{enumerate}
...
\item All \idx{approximate solution}s obtained by \autoref{pro:appsol} solve the linear system \(A\xv=\tilde\bv\) for the \idx{consistent} right-hand side vector~\(\tilde\bv\) that minimises the \idx{distance}~\(|\tilde\bv-\bv|\) (\autoref{thm:appsol}).

\item Obtain the \bfidx{smallest solution}, whether exact or as an approximation, to a system of \idx{linear equation}s by setting to \idx{zero} the \idx{free variable}s, \(y_{r+1}=\cdots=y_n=0\)\,, in Procedures~\ref{pro:gensol} and~\ref{pro:appsol} (\autoref{thm:smallsoln}).
...
\item Let \WW\ be a \(k\)-dimensional \idx{subspace} of~\(\RR^n\) with an \idx{orthonormal basis} \(\{\hlist\wv k\}\).
...
\item The `\idx{least square}' solution(s) of the system \(A\xv=\bv\) determined by \autoref{pro:appsol} is(are) the solution(s) of \(A\xv=\proj_{\AA}(\bv)\) where \AA~is the \idx{column space} of~\(A\) (\autoref{thm:lsqproj}).

\item Let \WW\ be a \(k\)-dimensional \idx{subspace} of~\(\RR^n\) with an \idx{orthonormal basis} \(\{\hlist\wv k\}\), then for every vector \(\vv\in\RR^n\), the \idx{orthogonal projection}
...
\item For every \(m\times n\) matrix~\(A\), and denoting the column space of~\(A\) by \(\AA=\Span\{\hlist\av n\}\), the orthogonal complement \(\AA^\perp=\Null(\tr A)\) (\autoref{thm:nulltrw}).
...
\item Let \WW\ be a \idx{subspace} of~\(\RR^n\) and vector \(\vv\in\RR^n\), then there exist unique vectors \(\wv\in\WW\) and \(\nv\in\WW^\perp\) such that vector \(\vv=\wv+\nv\) (\autoref{thm:odt}); this  sum is called an \bfidx{orthogonal decomposition} of~\vv.

\item For every vector~\vv\ in~\(\RR^n\), and every subspace~\WW\ in~\(\RR^n\),  \(\proj_\WW(\vv)\) is the closest vector in~\WW\ to~\vv\ (\autoref{thm:bapr}).
...
\item A \idx{transformation}\slash function \(T:\RR^n\to\RR^m\) is called a \bfidx{linear transformation} if (\autoref{def:lintran})
\begin{itemize}
...
\item For every \idx{linear transformation}  \(T:\RR^n\to\RR^m\),
the \(T\)~is the transformation corresponding to the \(m\times n\)  matrix (\autoref{thm:matlintran})
\begin{equation*}
...
\item For every system of linear equations \(A\xv=\bv\) with \(m\times n\) matrix~\(A\), \autoref{pro:appsol} forms a \idx{linear transformation} \(T:\RR^m\to\RR^n\) to find the smallest solution~\xv\ to the closest consistent system \(A\xv=\tilde\bv\).
This linear transformation~\(T\) has an \(n\times m\) \idx{standard matrix}~\(A^+\) called the \bfidx{pseudo-inverse}, or \bfidx{Moore--Penrose inverse}, of matrix~\(A\) (\autoref{thm:lsqlt}).
...
\item Let \(T:\RR^n\to\RR^n\) be an \idx{invertible} \idx{linear transformation}. 
