...
Recall that the \idx{standard unit vector}s in~\(\RR^3\) are \index{e@$\ev_j$}\(\ev_1=(1,0,0)\), \(\ev_2=(0,1,0)\) and \(\ev_3=(0,0,1)\) (\cref{def:stuniv}); so every other vector in~\(\RR^3\) may be \text{written as}
\begin{eqnarray*}
\xv&=&(x_1,x_2,x_3)
...
This expression is a parametric description of a \idx{parabola} in~\(\RR^3\), as illustrated above in stereo, and very soon we will be able to say it is a parabola in the plane spanned by \((-1,2,0)\) and~\((0,2,1)\).
\end{figbox}
