...
For example, the \idx{general solution} to \autoref{eg:homosysiv}  is 
\begin{eqnarray*}
...
Recall the \idx{standard unit vector}s in~\(\RR^3\) are \index{e@$\ev_j$}\(\ev_1=(1,0,0)\), \(\ev_2=(0,1,0)\) and \(\ev_3=(0,0,1)\) (\autoref{def:stuniv}): so any other vector in~\(\RR^3\) may be written as
\begin{eqnarray*}
\xv&=&(x_1,x_2,x_3)
...
\end{equation*}
Then use addition and \idx{scalar multiplication} of vectors (\autoref{def:vecops}) to rewrite the left-hand side vector as
\begin{equation*}
...
A \idx{system} of \idx{linear equation}s \(A\xv=\bv\) is \idx{consistent} (\autoref{pro:gje}) 
\begin{aside}
...
From the \idx{parametric equation} of a line (\autoref{def:parlin}), \(\Span S\) is all vectors in the line \(y=-2x\) as shown in the margin.
...
\begin{reduce}
\item For the complete set of \(n\)~\idx{standard unit vector}s in~\(\RR^n\) (\autoref{def:stuniv}), \index{e@$\ev_j$}\(\Span\{\hlist\ev n\}=\RR^n\).
...
\item The \idx{homogeneous} system (\autoref{def:homosys}) of linear equations from \autoref{eg:homosysiv} has solutions \(\xv=(-2s-\frac{15}7t,s,\frac97t,t) =(-2,1,0,0)s+(-\frac{15}7,0,\frac97,1)t\) for arbitrary \(s\) and~\(t\).
