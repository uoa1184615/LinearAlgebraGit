...
\begin{solution} 
The marginal plot shows the three planes represented by the given equations (in the order blue, brown, red), and plots the (black) point we seek of \idx{intersection} of all three planes.
\marginpar{\begin{tikzpicture}[]
...
\autoref{eg:infertemp} transformed the problem of \idx{inferring} a line through two data points into solving two linear equations.
...
Given a geometric problem of \idx{inferring} what plane passes through three given points, we transform this problem into the linear algebra task of finding the \idx{intersection} point of three specific planes.
This task we do.
...
The only three possibilities are: (a)~a unique solution; (b)~infinitely many solutions; and (c)~\idx{no solution}.}
\label{fig:3eq3var}}
& (c)
\begin{tikzpicture}[]
\begin{axis}[footnotesize,width=\wid
...
\autoref{fig:3eq3var} illustrates the three general possibilities: a \idx{unique solution} (as in \autoref{eg:3eq3var}), or infinitely many solutions, or \idx{no solution}.
