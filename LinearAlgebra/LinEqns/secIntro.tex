...
Linear equations are characterized by each unknown never being multiplied or divided by another unknown, or itself, or inside `curvaceous' functions.  
...
A \bfidx{linear equation} in the \(n\)~variables \hlist xn\ is an equation that can be written in the form
\begin{equation*}
\lincomb axn=b
\end{equation*}
...
\begin{solution} 
To draw the graphs seen in the plot to the right, rearrange  the linear equations as \(y=3-x\) and \(y=x/2\)\,.  
From the graph, they intersect only at the point~\((2,1)\) so \(x=2\) and \(y=1\) is the unique solution.
\notyetaqedfalse%fudge to help pagebreak
...
\item 
\begin{figbox}[\vspace{1ex}]{\begin{tikzpicture}[]
\begin{axis}[footnotesize
    , axis equal image, axis lines=middle,
    , xlabel={$x$},ylabel={$y$}, thick, no marks, grid
    , domain=-0.5:3.5 ]
...
\end{axis}
\end{tikzpicture}}%
\(\begin{array}{l} 2x-3y=2\\-4x+6y=3 \end{array}\)
...
Thus there are no solutions to \text{the system.}
\aqed
...
A receiver, say your smart-phone, picks up these signals and, from the time taken for the signals to arrive, knows the distance to all those satellites within `sight'.
Your smart-phone then solves a system of equations and informs you of its \text{precise position.}
...
However, straightforward algebra transforms these three \idx{nonlinear equation}s into a system of two linear equations which \text{we solve.}
...
\end{tikzpicture}
\end{wrapfigure}
The plot to the right shows the three planes represented by the given equations (in the order blue, brown, red), and plots the (black) point of \idx{intersection} of all three planes.
...
This geometric interpretation then empowers further methods useful \text{in applications.}
...
Nonetheless, it is the abstraction by linear algebra that empowers wonderful results \text{for applications.}


\begin{wraptable}[9]r{15em}
...
\end{tikzpicture}
\end{tabular}
\caption{\label{fig:3eq3var}Solving three linear equations in three variables finds the \idx{intersection} point(s) of three planes.
...
\begin{exercise}  
For each of the following graphs: estimate the equations of the pair of lines; solve the pair of equations algebraically; and confirm that the algebraic solution is reasonably close to the \idx{intersection} of the pair of lines.
\newcommand{\temp}{
