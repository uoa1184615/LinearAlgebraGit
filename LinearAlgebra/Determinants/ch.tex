...
The typical teaching of determinants starts from our endpoint: it starts with the Laplace Expansion \cref{thm:letdet} as a definition of determinants and then works algebraically through the same properties developed here, but roughly in reverse order.
...
Recall from previous study (\cref{sec:feebh}, e.g.)
\begin{itemize}
\item a \(2\times 2\) matrix \(A=\begin{bmat} a&b\\c&d \end{bmat}\) has \idx{determinant} \(\det A=|A|=ad-bc\)\,, and that the matrix~\(A\) is  \idx{invertible} if and only if \(\det A\neq0\)\,;
\item a \(3\times 3\) matrix \(A=\begin{bmat} a&b&c\\d&e&f\\g&h&i \end{bmat}\) has \idx{determinant} \(\det A=|A|=aei+bfg+cdh-ceg-afh-bdi\)\,, and that the matrix~\(A\) is \idx{invertible} if and only if \(\det A\neq0\)\,.
\end{itemize}
