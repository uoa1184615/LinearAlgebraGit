...
\item \index{std()@\texttt{std()}}\verb|std(A)| of an \(m\times n\) array computes the \(n\)~elements in the row vector of the \idx{standard deviation} over each column of~\(A\).
...
\item Define the \bfidx{matrix norm} (sometimes called the \idx{spectral norm}) such that for every \(m\times n\) matrix~\(A\), 
\begin{equation*}
