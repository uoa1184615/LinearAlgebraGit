...
\item In \script:
\begin{itemize}
\itemme \index{norm()@\texttt{norm()}}\verb|norm(A)| computes the \idx{matrix norm}, \autoref{def:norm}, namely the largest \idx{singular value} of the matrix~\(A\).
...
\item \index{bsxfun()@\texttt{bsxfun()}}\verb|bsxfun(@op,A,B)| for \(m\times n\) arrays~\(A\) and~\(B\) computes the \(m\times n\) array \(\begin{bmatrix} a_{ij}\operatorname{op}b_{ij} \end{bmatrix}\), where ``\verb|@op|'' is one of the operations~\verb|@plus|, \verb|@minus|, \verb|@times|, \verb|@rdivide|, \ldots.
Where this \verb|bsxfun()| becomes useful is that if either  of the matrices is \(m\times1\) or \(1\times n\) then its column\slash row is replicated to a matrix of size~\(m\times n\) and the operation then performed. 
...
\itemhi \autoref{pro:appmat} approximates \idx{linear equation}s.
...
Suppose the expected error in the matrix entries are of size~\(e\) (here ``\(e\)''~denotes ``error'', not the \idx{exponential}~\(e\))
\begin{enumerate}
...
the matrix~\(\tilde A\) is obtained from~\(A\) by replacing each of its nonzero \idx{singular value}s~\(\sigma_i\) by \(\tilde\sigma_i:=\sigma_i+\alpha^2/\sigma_i\) (\autoref{thm:Tikreg}).
