...
We establish that there are generally \(n\)~eigenvalues of an \(n\times n\) matrix, albeit possibly complex valued, and that \idx{repeated eigenvalue}s are sensitive to errors.
...
The distinction is immaterial as,  for an \(n\times n\) matrix~\(A\) and by \autoref{thm:basicdet:iii} with multiplicative factor \(k=-1\)\,, the only difference in the \idx{determinant} is a factor of~\((-1)^n\).
...
\end{aside}
The \idx{sum} of the \idx{eigenvalue}s equals \((-1)^{n-1}\)~times the \idx{coefficient} of~\(\lambda^{n-1}\) in the \idx{characteristic polynomial} and equals the \bfidx{trace} of the matrix, defined as the sum of the diagonal \idx{elements} \(a_{11}+a_{22}+\cdots+a_{nn}\)\,.
...
\item write each \idx{eigenspace} as the \idx{span} of a few chosen \idx{eigenvector}s  (\autoref{def:basis} calls such a set a \idx{basis}).
\end{enumerate}
...
the \idx{diagonal entries} of~\verb|D|, \index{diag()@\texttt{diag()}}\verb|diag(D)|, are the \idx{eigenvalue}s of~\verb|A|. 
...
Instead, to find eigenvalues and eigenvectors, just as for the \svd, \script\ uses an intriguing iteration based upon what is called the \idx{QR factorization}\ifcsname ch:qrfuma\endcsname\ (\autoref{ch:qrfuma})\fi.}
...
(and, as \autoref{sec:lisb} discusses, when the \idx{column vector}s have a property called \idx{linear independence} then they form a so-called \idx{basis} for the eigenspace). 
\end{compute}
...
Find the \idx{eigenspace}s corresponding to the eigenvalues found for the first three matrices of \autoref{eg:faem}.
\begin{description}
\item[\ref{eg:faem:a}]
...
The matrices in \autoref{eg:faespm} all have \idx{repeated eigenvalue}s.  
...
To confirm the hand calculation of eigenvectors in \autoref{eg:fiveev}, here divide each column of~\verb|V| by its last element,~\verb|V(5,:)|, via the \idx{auto-replication} in \verb|V./V(5,:)|, which gives the more appealing matrix of eigenvectors \twodp
\begin{verbatim}
ans =
...
Repeat some of the previous cases of \autoref{eg:multeval}, preceded by the \script\ command \index{format long@\texttt{format long}}\verb|format long|, to see that repeated eigenvalues are sensitive to computational~errors.
...
\\2&-1&4&1\end{bmatrix}
\end{equation*}
and see matrix~\(A\) has an eigenvalue of \idx{multiplicity} two.  
...
For a given \idx{species}, let's define \begin{itemize}
...
\marginpar{Although some bacteria\slash algae have seven sexes!}
...
However, to make predictions for many years and for general \idx{initial population}s we prefer the formula solution given by the upcoming \autoref{thm:dynsol} and introduced in the next example.
...
\begin{activity}
For \autoref{eg:2x2map}, what is the \idx{particular solution} when \(\yv(0)=(1,1)\)?
...
For what value(s) of~\(a\) does \autoref{thm:dynsol} \emph{not} provide a \idx{general solution} to \(\yv(t+1)=A\yv(t)\)?
...
\begin{itemize}
\item For example, in the \idx{general solution} for \autoref{eg:2x2map}, \(\yv(t)=c_1(-1)^t(1,2)+c_23^t(-1,2)\), the \(3^t\)~factor grows in time since \(3^1=3\), \(3^2=9\), \(3^3=27\), and so on.
...
\item In \autoref{eg:2x2ccgrow} with \idx{complex conjugate} eigenvalues the situation is more complicated.
...
\end{tikzpicture}}
\begin{itemize}
\item grow to \idx{infinity} if \(|\lambda|>1\)\,,
...
Extend the \idx{orangutan} analysis of \autoref{eg:orang}. 
Use \autoref{thm:dynsol} to predict the population over many years: from an \idx{initial population} of \(30\)~\idx{adolescent} \idx{female}s and \(15\)~\idx{adult} females; and from a general initial population.
\begin{solution} 
...
Crucial in this section---so that we find a solution for all \idx{initial value}s---is that the matrix of eigenvectors is \idx{invertible}.
...
(recall that \index{log}``\(\ln(x)\)'' denotes the \idx{natural logarithm} of~\(x\),  computed in \script\ with \index{log()@\texttt{log()}}\verb|log(x)|, see \autoref{tbl:mtlbmops}).
...
\end{eqnarray*}
These are \idx{nonlinear equation}s, but some algebraic tricks empower us to use our beautiful linear algebra to find the solution.
\marginpar{\mytemp1}%
...
We avoid this inversion by generalizing \idx{eigen-problem}s as introduced by the following reworking of parts of \autoref{eg:2expfit}.
...
Most importantly here, albeit without proof, provided matrix~\(B\) is invertible, then counted according to \idx{multiplicity} there are \(n\)~eigenvalues of a \idx{generalized eigen-problem} in \(n\times n\) matrices.
...
Since the eigenvalues~\(\lambda\) may be \idx{complex valued} we need the \idx{transpose}~``\verb|.'|'' not the \idx{complex conjugate} transpose~``\verb|'|'' (\autoref{tbl:mtlbops}).
\end{enumerate}
\end{procedure}
...
The command \index{eig()@\texttt{eig()}|textbf}\verb|[V,D]=eig(A,B)| solves the \idx{generalized eigen-problem} \(A\xv=\lambda B\xv\) for eigenvalues~\(\lambda\) returned in the diagonal of matrix~\(D\) (some may be~\index{Inf@\texttt{Inf}}\(\pm\verb|Inf|\)), and corresponding \idx{eigenvector}s returned in the corresponding column of~\(V\).
...
\item Compute the coefficients in the exponential fit with the following (\autoref{tbl:mtlbnorm} introduces \idx{auto-replication} of~\verb|.^|)
\begin{verbatim}
U=(lambda.^(0:3)).'
rcond(U)
c=U\f(1:4)
...
Sound statistical analysis needs to supplement \autoref{pro:ei} to inform us about expected errors and \idx{sensitivity}.
...
\begin{exercise}  
Reconsider the mathematical modelling of the \idx{serval}, \autoref{eg:serval}.
...
\begin{quoted}{\idx{Wikipedia}, 2016}
...
\begin{exercise}  
Use the properties of \idx{determinant}s (\autoref{ch:ddm}), and that an \(n\)th~degree polynomial has exactly \(n\)~zeros (when counted according to multiplicity), to explain why the \idx{generalized eigen-problem} \(A\vv=\lambda B\vv\), for real \(n\times n\) matrices~\(A\) and~\(B\), has \(n\)~eigenvalues iff matrix~\(B\) is \idx{invertible}.
\end{exercise}
...
Prove that if both~\(A\) and~\(B\) are real \emph{symmetric} \(n\times n\) matrices, then the eigenvalues of the \idx{generalized eigen-problem} \(A\vv=\lambda B\vv\) are all real.
...
%\end{verbatim}
\begin{exercise}  
Recall that \autoref{eg:orthbapp} introduced the phenomenon of \idx{El Ni\~no} which makes a large impact on the world's weather.
...
\autoref{fig:soiRoundData} plots a (`smoothed') yearly \idx{average} \soi\ each year for fifty years up to 1993.
