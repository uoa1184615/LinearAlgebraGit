...
\end{aside}
The sum of the \idx{eigenvalue}s equals \((-1)^{n-1}\)~times the \idx{coefficient} of~\(\lambda^{n-1}\) in the \idx{characteristic polynomial} and equals the \bfidx{trace} of the matrix, defined as the sum \(a_{11}+a_{22}+\cdots+a_{nn}\)\,.
...
(Since this is an upper triangular matrix, Theorem~\ref{thm:trieig} asserts the eigenvalues are simply the diagonal elements, namely~\(3\) twice.)
\end{solution}
...
\item for each \idx{eigenvalue}~\(\lambda\), solve the homogeneous linear equation \((A-\lambda I)\xv=\ov\) to find the \idx{eigenspace}~\(\EE_\lambda\) of all \idx{eigenvector}s (together with~\ov);
...
(and, as Section~\ref{sec:lisb} discusses, when the column vectors have a property called \idx{linear independence} then they form a so-called \idx{basis} for the eigenspace). 
\end{compute}
...
\end{equation*}
(Example~\ref{eg:fiveevm} confirms the answer using \verb|eig()| in \script.)
\begin{solution} 
...
The columns of eigenvectors are more easily seen to confirm the hand calculation of Example~\ref{eg:fiveev} when we divide each column by its last element,~\verb|V(5,:)|, via the post-multiplication by the diagonal matrix in \verb|V*diag(1./V(5,:))| which gives the more appealing \twodp
\begin{verbatim}
ans =
...
1.0850.
\end{equation*}
What do you expect is the number of different eigenvalues?
\partswidth=5em
\begin{parts}
\item two
\item three
...
However, to make predictions for many years and for general initial populations we prefer the formula solution given by the upcoming Theorem~\ref{thm:dynsol} and introduced in the next example.
...
\begin{activity}
For Example~\ref{eg:2x2map}, what is the particular solution when \(\yv(0)=(1,1)\)?
\begin{enumerate}
...
Since matrix~\(P\) is invertible, \(P\cv=\yv(0)\) always has the unique solution \(\cv=P^{-1}\yv(0)\) (Theorem~\ref{thm:ftim2}) which determines the requisite constants.
\end{itemize}
\end{proof}
...
For what value(s) of~\(a\) does Theorem~\ref{thm:dynsol} \emph{not} provide a general solution to \(\yv(t+1)=A\yv(t)\)?
\partswidth=5em
\begin{parts}
\item \(a=-1\)
\item \(a=0\)\actans
...
Extend the orangutan analysis of Example~\ref{eg:orang}. 
Use Theorem~\ref{thm:dynsol} to predict the population over many years: from an \idx{initial population} of \(30\)~adolescent females and \(15\)~adult females; and from a general initial population.
\begin{solution} 
...
Solve this linear equation in \script\ with Procedure~\ref{pro:unisol}:
\begin{verbatim}
y0=[0;30;15]
rcond(V)
c=V\y0
...
The next Section~\ref{sec:lisb} relates the invertibility of a matrix of eigenvectors to the new concept of `linear independence' (Theorem~\ref{thm:ftim3}).
...
Given an \(m\times n\) matrix~\(A\), the \idx{singular value}s of~\(A\) are the non-negative \idx{eigenvalue}s of the \((m+n)\times(m+n)\) \idx{symmetric matrix} \(B=\begin{bmatrix} O_m&A\\\tr A&O_n \end{bmatrix}\). 
...
Then the fundamental eigen-problem \(B\wv=\lambda\wv\)  (Definition~\ref{def:evecval}) separates into
\begin{equation}
...
\footnote{Some of you will know that identification of frequencies is most commonly done by what is called a Fourier transform.  
...
\begin{exercise} \label{ex:} 
Reconsider the mathematical modelling of the serval, Example~\ref{eg:serval}.
