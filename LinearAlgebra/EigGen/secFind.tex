...
Given the additional determinant methods of \autoref{ch:ddm}, this section begins exploring the properties and some applications of the \idx{eigen-problem} \(A\xv=\lambda\xv\) for general matrices~\(A\).
...
This contrasts with the case of symmetric matrices where the eigenspaces always have the same dimensionality as the \idx{multiplicity} of the eigenvalue, as illustrated by Examples~\ref{eg:espace2d} and~\ref{eg:sier2eig}.
...
This \(A\wv=\lambda B\wv\) forms a \idx{generalised eigen-problem} because it reduces to the standard \idx{eigen-problem} in cases when the matrix \(B=I\).
...
\index{eigen-problem!generalised|(}
...
The \bfidx{generalised eigen-problem} is to find \idx{scalar} \idx{eigenvalue}s~\(\lambda\) and corresponding nonzero \idx{eigenvector}s~\vv\ such that \(A\vv=\lambda B\vv\).
\end{definition}
...
\end{itemize}
This \idx{generalised eigen-problem} typically determines \(n\)~multipliers \hlist\lambda n, and thence the \(n\)~rates \(r_k=(\ln\lambda_k)/h\)\,.

\item\label{pro:eic} Determine the corresponding \(n\)~\idx{coefficient}s \hlist cn\ from any \(n\)~point subset of the \(2n\)~data points.
...
\end{equation*}
In \script\ one may construct the matrix~\(U\) appearing here with \index{bsxfun()@\texttt{bsxfun()}}\verb|U=bsxfun(@power,lambda,0:n-1).'| when \verb|lambda| is a \idx{column vector} of the eigenvalues.
...
The command \index{eig()@\texttt{eig()}|textbf}\verb|[V,D]=eig(A,B)| solves the \idx{generalised eigen-problem} \(A\xv=\lambda B\xv\) for eigenvalues~\(\lambda\) returned in the diagonal of matrix~\(D\) (some may be~\index{Inf@\texttt{Inf}}\(\pm\verb|Inf|\)), and corresponding \idx{eigenvector}s returned in the corresponding column of~\(V\).
...
The techniques and theory of this subsection generalise to cater for noisy data and for complex \idx{system} interactions with vast amounts of data.
...
\begin{exercise} \label{ex:} 
Use the properties of \idx{determinant}s (\autoref{ch:ddm}), and that an \(n\)th~degree polynomial has exactly \(n\)~zeros (when counted according to multiplicity), to explain why the \idx{generalised eigen-problem} \(A\vv=\lambda B\vv\), for real \(n\times n\) matrices~\(A\) and~\(B\), has \(n\)~eigenvalues iff matrix~\(B\) is \idx{invertible}.
\end{exercise}
...
invent real symmetric matrices~\(A\) and~\(B\) such that the \idx{generalised eigen-problem} \(A\vv=\lambda B\vv\) has \idx{complex valued} eigenvalues.
