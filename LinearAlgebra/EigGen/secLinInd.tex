...
In \autoref{ch:eesm} on symmetric matrices, the \idx{eigenvector}s from \idx{distinct eigenvalues} are proved to be always \idx{orthogonal}---because of the symmetry.  
...
Answer: yes. 
We now extend ``orthogonal'' to the more general concept of ``linear independence''.
...
Analogously, the concept of linear independence is closely connected to \idx{coordinate system}s that are not orthogonal.



\paragraph{Subspace \idx{coordinate system}s} 
In any given problem we want two things from a \idx{general solution}: 
\begin{itemize}
...
We replace orthonormal \idx{unit vector}s by any suitable set of basis vectors.
...
As illustrated in the margin, every point in the plane (end point of a vector) is a unique \idx{linear combination} of the two drawn basis vectors~\(\vv_1\) and~\(\vv_2\).
...
\begin{activity}
Write the vector shown in the margin as a \idx{linear combination} of vectors~\(\vv_1\) and~\(\vv_2\).
\marginpar{\begin{tikzpicture}
\begin{axis}[footnotesize,font=\footnotesize
  ,axis lines=middle, axis equal image
  ,xmax=2.3,ymax=2.3
...
A set of vectors that is not linearly dependent is called \bfidx{linearly independent} (characterized by \emph{only} the linear combination with \(c_1=c_2=\cdots=c_k=0\) gives the \idx{zero vector}).
\end{definition}
...
\end{enumerate}
These last two cases generalize to the next \autoref{thm:ortholi} about the linear independence of every \idx{orthonormal set} of vectors.
\end{example}
...
This linear dependence of proportional vectors then generalizes in the forthcoming \autoref{thm:lindeplc}.
...
The following \autoref{thm:indepev} generalizes this property to non-symmetric matrices using the concept of linear independence.
...
For what value of~\(a\) does the matrix have a \idx{repeated eigenvalue}?
...
\\-1&0&-1
\\0&-3&1 \end{bmat}\) from \autoref{eg:faespm}.
\begin{solution} 
In \script, executing 
\begin{verbatim}
B=[-1 1 -2
...
\item\label[example]{eg:indepeva} \autoref{eg:fiveev} found the eigenvalues and eigenvectors of matrix
\begin{equation*}
...
Because the five singular values of square matrix~\(V\) are all nonzero,\footnote{One could alternatively compute the determinant \(\texttt{det(V)}=0.09090\) and because it is nonzero \autoref{thm:ftim3} asserts that the equation has only the solution \(\cv=\ov\)\,.} obtained from \verb|svd(V)|~as
\begin{verbatim}
ans =
   1.7703
   1.1268
...
This last case of \autoref{eg:indepeva} connects the concept of linear in/dependence to the existence or otherwise of nonzero solutions to a homogeneous system of \idx{linear equation}s, \(V\cv=\ov\)\,.
...
Recall \autoref{thm:orthcomp} that in~\(\RR^n\) there can be no more that \(n\)~vectors in an \idx{orthogonal set} of vectors.
...
The zero singular value implies there is a free variables when solving \(V\cv=\ov\) (\autoref{pro:gensol}), and consequently there are infinitely many nonzero~\cv\ that solve \(V\cv=\ov\).
...
As the singular values are all nonzero, the homogeneous system \(V\cv=\ov\) has the unique solution \(\cv=\ov\) (\autoref{pro:gensol}), and hence the set of five vectors are linearly independent.
...
Recall the definition of \idx{subspace}s and the \idx{span}, from 
...
\begin{example} 
\begin{enumerate}
\item Recall \cref{eg:lindepb,eg:2dnoc} showed that the two vectors \((2,1)\) and \((1,2)\) are linearly independent and span~\(\RR^2\). 
...
\begin{activity}
Which of the following sets of vectors form a basis for~\(\RR^2\), but is not an \idx{orthonormal basis} for~\(\RR^2\)?
\def\temp#1{\begin{tikzpicture}
\begin{axis}[footnotesize,font=\footnotesize
  ,axis lines=none, axis equal image
...
The proof directly generalizes that for \autoref{thm:sameD}.
...
\item Factorize~\(A\) into its \svd, $A=\usv$\,, and let \(r=\rank A\) be the number of nonzero \idx{singular value}s (or effectively nonzero when the matrix has \idx{experimental error}s, \autoref{sec:rle}).
...
\item  Adapting \autoref{pro:gensol} for the specific case of homogeneous systems, first find an \svd\ \idx{factorization} \(A=\usv\) and let \(r=\rank A\) be the number of nonzero \idx{singular value}s (or effectively nonzero when the matrix has \idx{experimental error}s, \autoref{sec:rle}).
...
In the orthogonal \idx{standard coordinate system} these three vectors and the origin have coordinates as plotted on the left by their end-points.
...
and \(\av=2\vv_1+\vv_2\) without any apparent contradiction: these 
two statements explicitly recognize the underlying \idx{standard unit 
vector}s in the first expression and the underlying \idx{non-orthogonal basis} vectors in the second.
...
The endemic practice of just writing \(\av=(2,1)\), \(\bv=(1,-3)\) and \(\cv=(-2,3)\) is rationalized in this more precise notation by the convention that if no basis is explicitly specified, then we assume the \idx{standard basis}~\cE.%
...
The answer is yes:  for a given \idx{linear transformation} (\autoref{sec:ilt}), the numbers in the \idx{components} of its matrix also depend upon the coordinate basis. }
\end{example}



\begin{theorem} \label{thm:ssbc} 
For any \idx{subspace}~\WW\ of~\(\RR^n\) let \(\cB=\{\hlist\vv k\}\) be a \idx{basis} for~\WW.  
...
\begin{example} 
Let the basis \(\cB=\{\vv_1,\vv_2,\vv_3\}\) for the three given vectors \(\vv_1=(-1,1,-1)\), \(\vv_2=(1,-2,0)\) and \(\vv_3=(0,4,5)\) (each of these are specified in the \idx{standard basis}~\cE\ of the \idx{standard unit vector}s~\(\ev_1\), \(\ev_2\) and~\(\ev_3\)).
\begin{enumerate}
...
\end{theorem}
\begin{proof} 
Recall \cref{thm:ftim1,thm:ftim2} established the equivalence of \ref{thm:ftim3i}--\ref{thm:ftim3viii}, and \autoref{thm:detinv} proved the equivalence of \ref{thm:ftim3i} and~\ref{thm:ftim3ix}.
...
Prove the particular case of \autoref{thm:lindeplc}, namely that a 
...
Recall that \autoref{thm:lenlam} establishes there are at most \(n\)~\idx{eigenvalue}s of an \(n\times n\) symmetric matrix.
Adapt the proof of that theorem, using linear independence, to prove there are at most \(n\)~eigenvalues of an \(n\times n\) \idx{non-symmetric matrix}.
...
Let the three vectors \(\bv_1=(-1,-2,5,3,-2)\), \(\bv_2=(-2,-2,2,-1,-1)\) and \(\bv_3=(-4,6,-4,2,-1)\) form a \idx{basis} \(\cB=\{\bv_1,\bv_2,\bv_3\}\) for the \idx{subspace}~\BB\ in~\(\RR^5\) (specified in the \idx{standard basis}~\cE).
\setbox\ajrqrbox\hbox{\qrcode{% basis
B=[-1 -2 -4
 -2 -2 6
 5 2 -4
