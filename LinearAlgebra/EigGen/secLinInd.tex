...
We replace the orthonormal \idx{standard unit vector}s by any suitable set of basis vectors.
...
A set of vectors that is not linearly dependent is called \bfidx{linearly independent} (characterised by \emph{only} the linear combination with \(c_1=c_2=\cdots=c_k=0\) gives the \idx{zero vector}).
\end{definition}
...
\end{enumerate}
These last two cases generalise to the next \autoref{thm:ortholi} about the linear independence of every \idx{orthonormal set} of vectors.
\end{example}
...
Sections~\ref{sec:lcss} and~\ref{sec:sbd}: namely that a subspace is 
...
\item Factorise~\(A\) into its \svd, $A=\usv$\,, and let \(r=\rank A\) be the number of nonzero \idx{singular value}s (or effectively nonzero when the matrix has \idx{experimental error}s, \autoref{sec:rle}).
...
\item  Adapting \autoref{pro:gensol} for the specific case of homogeneous systems, first find an \svd\ \idx{factorisation} \(A=\usv\) and let \(r=\rank A\) be the number of nonzero \idx{singular value}s (or effectively nonzero when the matrix has \idx{experimental error}s, \autoref{sec:rle}).
...
\begin{example}[a tale of two \idx{coordinate system}s] \label{eg:}
...
two statements explicitly recognise the underlying \idx{standard unit 
vector}s in the first expression and the underlying \idx{non-orthogonal basis} vectors in the second.

Consequently we invent a new more precise notation.
...
\item\label{thm:ftim3ii} \(A\xv=\bv\) has a \bfidx{unique solution} for every \(\bv\in\RR^n\);
...
\item\label{thm:ftim3v} \index{rank}\(\rank A=n\)\,;
\item\label{thm:ftim3vi} \index{nullity}\(\nullity A=0\)\,;
...
\item\label{thm:ftim3x} \(0\) is not an \idx{eigenvalue} of~\(A\);
\item\label{thm:ftim3xi} the \(n\) column vectors of~\(A\) are linearly independent;
\item\label{thm:ftim3xii} the \(n\) \idx{row vector}s of~\(A\) are linearly independent.
\end{enumerate}
\end{theorem}
\begin{proof} 
