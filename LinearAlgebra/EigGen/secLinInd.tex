...
In \cref{ch:eesm} on symmetric matrices, the \idx{eigenvector}s from \idx{distinct eigenvalues} are proved to be always \idx{orthogonal}---because of the matrix symmetry.  
...
\def\tmpp#1#2{\parbox[t]{13em}{\raggedright #2} \temp{#1}}
\begin{enumerate}
...
\begin{activity}
The vector shown in the margin is which of the following \idx{linear combination}s of shown vectors~\(\vv_1\) and~\(\vv_2\)?
\marginpar{\begin{tikzpicture}
\begin{axis}[footnotesize,font=\footnotesize
  ,axis lines=middle, axis equal image
  ,xmax=2.3,ymax=2.3
...
It is this last statement that leads to the \cref{def:lindep} of \idx{linearly dependent} vectors.
\end{solution}
\end{example}
...
\end{tikzpicture}}
\begin{solution} 
Since they are not proportional to each other, we cannot write either as a scalar multiple of the other, and so the pair are linearly independent. 
\end{solution}
...
For what values of~\(a\) does the matrix have a \idx{distinct eigenvalues}?
\actposs[4]{\(a\neq0\)}{\(a\neq-1\)}{\(a\neq1\)}{\(a\neq2\)}
\end{activity}
...
D =
   3.00      0      0      0      0
      ...
      0      0      0      0   1.73
\end{verbatim}
The five eigenvalues are all distinct, so \cref{thm:linhomo} asserts a set of corresponding eigenvectors will be linearly independent.
...
ans =
   ...
   0.1922
\end{verbatim}
consequently \cref{thm:ftim2} asserts \(V\cv=\ov\) has only the zero solution.
Hence, by \cref{def:lindep} the set of eigenvectors in the columns of~\(V\) are linearly independent. 

\end{enumerate}
\end{example}


This last case of \cref{eg:indepeva} connects the concept of linear in/dependence to the existence or otherwise of nonzero solutions to a homogeneous system of \idx{linear equation}s, \(V\cv=\ov\)\,.
...
The next \cref{thm:linhomo} establishes this connection in general.
...
The zero singular value implies there is a free variables when solving \(V\cv=\ov\) (\cref{pro:gensol}), and consequently there are infinitely many nonzero~\cv\ that solve \(V\cv=\ov\).
By \cref{thm:linhomo}, the set of vectors is linearly dependent.
\end{solution}

\item \(\{(10,3,3,1)\), \(( 2,-3,0,-1)\), \(( 1,-1,2,-1)\), \(( 2,-1,-3,0)\), \((-2,0,2,-1)\}\)
\begin{solution} 
As there are five vectors in~\(\RR^4\) so \cref{thm:mgtnli} asserts the set is linearly dependent.
\end{solution}
\end{reduce}
...
Also, \cref{def:orthobasis} defined an ``\idx{orthonormal basis}'' for a subspace to be a set of orthonormal vectors that span a subspace.
...
Recall \cref{pro:ospan} underpins finding an \idx{orthonormal basis} by the following.
\begin{enumerate}
...
Recall this \cref{sec:lisb} started by discussing the need to have a unique representation of solutions to problems.
...
The answer is yes:  for a given \idx{linear transformation} (\cref{sec:ilt}), the numbers in the \idx{components} of its matrix also depend upon the underlying coordinate basis. }
\end{example}
