...
In Chapter~\ref{ch:eesm} on symmetric matrices, the \idx{eigenvector}s from distinct \idx{eigenvalue}s are proved to be always \idx{orthogonal}.  
...
Every \idx{orthonormal set} (\autoref{def:orthoset}) of vectors is \idx{linearly independent}.
\end{theorem}
...
\end{axis}
\end{tikzpicture}}
\actposs{\temp2}{\temp1}{\temp3}{None of these sets.}
%\begin{parts}
%\item \temp1
%\item \temp2
%\item \temp3 \actans
...
\item  Adapting \autoref{pro:gensol} for the specific case of homogeneous systems, first find an \svd\ factorisation \(A=\usv\) and let \(r=\rank A\) be the number of nonzero \idx{singular value}s (or effectively nonzero when the matrix has experimental errors, Section~\ref{sec:rle}).
