...
In Chapter~\ref{ch:eesm} on symmetric matrices, the \idx{eigenvector}s from distinct \idx{eigenvalue}s are \idx{orthogonal}.  
...
Every \idx{orthonormal set} (Definition~\ref{def:orthoset}) of vectors is \idx{linearly independent}.
\end{theorem}
...
\ifnum#1>1\addplot[blue,quiver={u=-0.5,v=1},-stealth,thick] coordinates {(0,0)};\fi
...
Recall, from Sections~\ref{sec:lcss} and~\ref{sec:sbd}, the definition of \idx{subspace}s and the \idx{span}: namely that a subspace is a set of vectors closed under addition and scalar multiplication; and a span is a set of vectors whose linear combinations give all vectors in a given subspace.
...
\begin{solution} 
Theorem~\ref{thm:ortholi} establishes that every orthonormal basis is linearly independent.
...
Every two bases for a given \idx{subspace} have the same number of vectors.
\end{theorem}
\begin{proof} 
...
\item  Adapting Procedure~\ref{pro:gensol} for the specific case of homogeneous systems, first find an \svd\ factorisation \(A=\usv\) and let \(r=\rank A\) be the number of nonzero \idx{singular value}s (or effectively nonzero when the matrix has experimental errors, Section~\ref{sec:rle}).
