...
For each of the following matrices, show the eigenvectors from \idx{distinct eigenvalues} form linearly independent sets.
\begin{enumerate}
...
Recall the definition of \idx{subspace}s and the \idx{span}, from Sections~\ref{sec:lcss} and~\ref{sec:sbd}: namely that a subspace is a set of vectors closed under addition and scalar multiplication; and a span gives a subspace as all linear combinations of a set of vectors.
...
\item  Adapting \autoref{pro:gensol} for the specific case of homogeneous systems, first find an \svd\ factorisation \(A=\usv\) and let \(r=\rank A\) be the number of nonzero \idx{singular value}s (or effectively nonzero when the matrix has \idx{experimental error}s, \autoref{sec:rle}).
...
The answer is yes:  for a given \idx{linear transform}ation (\autoref{sec:ilt}), the numbers in the components of its matrix also depend upon the coordinate basis. }
\end{example}
