...
The answer is yes, in most cases: using \idx{eigenvalue}s and \idx{eigenvector}s the product~\(P^{-1}AP\) usually can be made into a simple \idx{diagonal matrix}.
...
\begin{solution} Invoke the original \cref{def:evecval} of an eigenvector for a matrix.
\begin{itemize}
...
Since matrix~\(P\) is invertible, \cref{thm:ftim3xi} asserts the \(n\)~columns vectors \hlist\pv n\ are linearly independent.
...
Recall the \idx{Sierpinski network} of \cref{eg:sier2eig} (shown in the margin).
Is the following \(9\times9\) matrix~\(A\) encoding the network diagonalizable?
...
Further, \cref{thm:gendiag} establishes that the matrix formed from the columns of the five eigenvectors will be a possible matrix
\begin{equation*}
...
\cref{thm:gendiag} then establishes the matrix~\(A\) is diagonalizable.

Second, since an \(n\times n\) matrix has \(n\)~eigenvalues, when counted accordingly to multiplicity and allowing for complex-valued eigenvalues (\cref{pro:geneig}), a non-diagonalizable matrix must have at least one repeated eigenvalue.  
Further, by \cref{thm:symspec} a real symmetric matrix is always diagonalizable: hence a non-diagonalizable matrix must also be non-symmetric.
\end{proof}
...
\begin{solution} 
The matrix appears to have only four distinct eigenvalues (two of them non-real \idx{complex valued}), and so on the given information \cref{thm:dlamd} cannot determine whether the matrix is diagonalizable or not.
...
However, for general non-symmetric matrices this equality between multiplicity and eigenspace dimension does not necessarily hold.
...
Although due to round-off error \script\ will report these with errors of about~\(10^{-5}\) (\cref{sec:reas})---possibly complex errors in which case ignore small complex parts.
...
Such differential equations are also common throughout engineering and science.
...
The forthcoming \cref{thm:ddtsol} confirms that the same approach solves general systems of differential equations: it corresponds to \cref{thm:dynsol} for discrete dynamics.
...
Use \idx{Euler's formula}, that \(e^{\i\theta}=\cos\theta+\i\sin\theta\) for every~\(\theta\), to rewrite the general solution of \cref{eg:decis2t} in terms of real functions.
\begin{solution}
