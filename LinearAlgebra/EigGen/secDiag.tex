...
The answer in most cases is yes: using \idx{eigenvalue}s and \idx{eigenvector}s the product~\(P^{-1}AP\) usually can be made into a simple \idx{diagonal matrix}.
...
Recall that (\autoref{sec:smod}) for a \idx{symmetric matrix}~\(A\) we could always factor \(A=VD\tr V=VDV^{-1}\) for \idx{orthogonal matrix}~\(V\) and \idx{diagonal matrix}~\(D\): thus a symmetric matrix is always \idx{orthogonally diagonalizable} (\autoref{def:odsble}).
For non-symmetric matrices, a \idx{diagonalization} mostly (although not always) can be done:
the difference being we need an invertible matrix, typically called~\(P\), instead of the orthogonal matrix~\(V\).
Such a matrix~\(A\) is termed `diagonalizable' instead of `\idx{orthogonally diagonalizable}'.
...
\item\label[example]{eg:diagonaliseb}\(B=\begin{bmat} 0&1\\0&0 \end{bmat}\) is not diagonalizable.
\begin{solution} 
Assume \(B\) is diagonalizable by the invertible matrix \(P=\begin{bmat} a&b\\c&d \end{bmat}\).
Being invertible, \(P\)~has inverse \(P^{-1}=\frac1{ad-bc}\begin{bmat} d&-b\\-c&a \end{bmat}\)  (\autoref{thm:2x2det}).
Then the product
\begin{equation*}
...
This \idx{contradiction} means that matrix~\(B\) is not diagonalizable.
\end{solution}
...
As a prelude to the next \autoref{thm:gendiag}, here show that the columns of~\(P\) are \idx{eigenvector}s of~\(A\).
...
This matrix is not diagonalizable as it has only one \idx{linearly independent} eigenvector, such as~\((1,0)\) or any nonzero multiple, and it needs two to be diagonalizable. 
...
Thus the matrix has only two \idx{linearly independent} eigenvectors, one from each eigenspace, and it needs three to be diagonalizable.
...
Use the results of \autoref{eg:fiveev} to show the following matrix is \idx{diagonalizable}:
\begin{equation*}
...
The \(5\times 5\) matrix in \autoref{eg:fiveevp} has five distinct eigenvalues whose corresponding \idx{eigenvector}s are necessarily \idx{linearly independent} (\autoref{thm:indepev}) and so diagonalize the matrix (\autoref{thm:gendiag}).
The \(3\times 3\) matrix in \autoref{eg:trieigp} has three distinct eigenvalues whose corresponding eigenvectors are necessarily \idx{linearly independent} (\autoref{thm:indepev}) and so diagonalize the matrix (\autoref{thm:gendiag}).
However, the matrices of \cref{eg:sier2eigp,eg:faespmp} have \idx{repeated eigenvalue}s---eigenvalues of \idx{multiplicity} two or more---and these matrices may (\autoref{eg:sier2eigp}) or may not (\autoref{eg:faespmp}) be diagonalizable.
The following theorem confirms that matrices with as many \emph{distinct} eigenvalues as the \idx{size} of the matrix are always diagonalizable.
...
Consequently, and allowing \idx{complex eigenvalue}s, a real \idx{non-diagonalizable} matrix must be non-symmetric and must have at least one \idx{repeated eigenvalue} (an \idx{eigenvalue} with \idx{multiplicity} two or~more).
\end{theorem}
\begin{proof}  
...
\begin{solution} 
The matrix appears to have only four distinct eigenvalues (two of them \idx{complex valued}), and so on the given information \autoref{thm:dlamd} cannot determine whether the matrix is diagonalizable or not.
...
\item Fourth, having diagonalized the matrix, expand this diagonalized set of differential equations to write this system in terms of \idx{components}: 
\begin{equation*}
...
Use \idx{Euler's formula}, that \(e^{\i\theta}=\cos\theta+\i\sin\theta\) for every~\(\theta\), to rewrite the general solution of \autoref{eg:decis2t} in terms of real functions.
\begin{solution}
...
Write down three different matrices~\(P\) that will diagonalize the matrix~\(A\), and for each write down the corresponding \idx{diagonal matrix} \(D=P^{-1}AP\).
\begin{enumerate}
