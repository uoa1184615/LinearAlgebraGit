...
Recall that (section~\ref{sec:smod}) for a \idx{symmetric matrix}~\(A\) we could always factor \(A=VD\tr V=VDV^{-1}\) for \idx{orthogonal matrix}~\(V\) and \idx{diagonal matrix}~\(D\): thus a symmetric matrix is always \idx{orthogonally diagonalisable} (\autoref{def:odsble}).
...
Recall \autoref{eg:trieig} found the \idx{triangular matrix}
\begin{equation*}
...
But further, by \autoref{thm:symspec} a symmetric matrix is always diagonalisable: hence a non-diagonalisable matrix must also be non-symmetric.
\end{proof}
