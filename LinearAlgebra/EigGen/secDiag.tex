...
\index{diagonalisable|(}

\paragraph{Population modelling} \index{population modelling}
Recall that this \autoref{ch:gee} started by introducing the dynamics of two interacting \idx{species} of animals.
Recall we let \(y(t)\) and~\(z(t)\) be the number of \idx{female} animals in each of the species at time~\(t\) (years). 
...
The answer is yes: using \idx{eigenvalue}s and \idx{eigenvector}s, in most cases the product~\(P^{-1}AP\) can be made into a simple \idx{diagonal matrix}.
...
Such a matrix~\(A\) is termed `diagonalisable' instead of `\idx{orthogonally diagonalisable}'.
...
This \idx{contradiction} means that matrix~\(B\) is not diagonalisable.
\end{solution}
...
As a prelude to the next \autoref{thm:gendiag}, show that the columns of~\(P\) are \idx{eigenvector}s of~\(A\).
...
Given matrix~\(F=\begin{bmatrix} 5&8\\-4&-7 \end{bmatrix}\) has eigenvectors \((-1,1)\) and~\((2,-1)\) corresponding to respective {eigenvalue}s~\(-3\) and~\(1\), what matrix diagonalises~\(F\) to \(D=\diag(-3,1)\)?
...
If \(A\)~is \idx{diagonalisable}, with \idx{diagonal matrix} \(D=P^{-1}AP\), then  the \idx{diagonal entries} of~\(D\) are \idx{eigenvalue}s, and the columns of~\(P\) are corresponding \idx{eigenvector}s.
\end{theorem}
\begin{proof} 
...
Recall the \idx{Sierpinski network} of \autoref{eg:sier2eig} (shown in the margin).
...
Recall \autoref{eg:faespm} found \idx{eigenvalue}s and corresponding \idx{eigenspace}s for various matrices.
Revisit these cases and show none of the matrices are diagonalisable.
\begin{enumerate}
\item Matrix \(A=\begin{bmatrix} 3&1\\0&3 \end{bmatrix}\) had one eigenvalue \(\lambda=3\) with \idx{multiplicity} two and corresponding eigenspace \(\EE_3=\Span\{(1,0)\}\).
This matrix is not diagonalisable as it has only one \idx{linearly independent} eigenvector, such as~\((1,0)\) or any nonzero multiple, and it needs two to be diagonalisable. 
...
Thus the matrix has only two \idx{linearly independent} eigenvectors, one from each eigenspace, and it needs three to be diagonalisable.
...
With only one \idx{linearly independent} eigenvector, the matrix is not diagonalisable.
...
\end{bmatrix}.
\end{equation*}
(One could also scale each column of~\(P\) by a different arbitrary nonzero constant, and the diagonalisation still holds.)
...
The \(5\times 5\) matrix in \autoref{eg:fiveevp} has five distinct eigenvalues whose corresponding \idx{eigenvector}s are necessarily \idx{linearly independent} (\autoref{thm:indepev}) and so diagonalise the matrix (\autoref{thm:gendiag}).
The \(3\times 3\) matrix in \autoref{eg:trieigp} has three distinct eigenvalues whose corresponding eigenvectors are necessarily \idx{linearly independent} (\autoref{thm:indepev}) and so diagonalise the matrix (\autoref{thm:gendiag}).
...
The following theorem confirms that matrices with as many \emph{distinct} eigenvalues as the \idx{size} of the matrix are always diagonalisable.
...
For every \(n\times n\) \idx{square matrix}~\(A\), if~\(A\) has \(n\)~\idx{distinct eigenvalues}, then \(A\)~is \idx{diagonalisable}.
Consequently, and allowing \idx{complex eigenvalue}s, a real \idx{non-diagonalisable} matrix must be non-symmetric and must have at least one \idx{repeated eigenvalue} (an \idx{eigenvalue} with \idx{multiplicity} two or~more).
\end{theorem}
\begin{proof}  
...
\begin{solution} 
The matrix appears to have only four distinct eigenvalues (two of them \idx{complex valued}), and so on the given information \autoref{thm:dlamd} cannot determine whether the matrix is diagonalisable or not.
...
\footnote{Nonetheless, in an application where errors are significant then the matrix may be effectively \idx{non-diagonalisable}.
...
Recall that for every \idx{symmetric matrix}, from \autoref{def:eigsymult}, the \idx{dimension} of an \idx{eigenspace},~\(\dim\EE_{\lambda_j}\) is equal to the \idx{multiplicity} of the corresponding eigenvalue~\(\lambda_j\).
...
\begin{example} \label{eg:}
Show the following matrix has one \idx{eigenvalue} of \idx{multiplicity} three, and the corresponding \idx{eigenspace} has \idx{dimension} two:
\begin{equation*}
...
As discussed by \autoref{sec:reas}, \idx{repeated eigenvalue}s are very sensitive and this \idx{sensitivity} means small variations in the hidden \script\ algorithm may produce quite large changes in the matrix~\verb|V| for repeated eigenvalues.
...
The \idx{population modelling} seen so far (\autoref{sec:ddp}) expressed the changes of the population over discrete intervals in time via discrete time equations such as \(y(t+1)=\cdots\) and \(z(t+1)=\cdots\)\,.
...
Let's start with a continuous time version of the \idx{population modelling} discussed at the start of this \autoref{ch:gee}.
...
The procedure is to choose the columns of~\(P\) to be \idx{eigenvector}s of the matrix~\(A\) (\autoref{thm:gendiag}).

\item Third, find the eigenvectors of~\(A\) by hand as it is a \(2\times2\) matrix. 
Here the matrix \(A=\begin{bmatrix} 1&-4\\-1&1 \end{bmatrix}\) has \idx{characteristic polynomial} \(\det(A-\lambda I)=(1-\lambda)^2-4\)\,.
...
\item Fourth, having diagonalised the matrix, expand this diagonalised set of differential equations to write this system in terms of \idx{components}: 
\begin{equation*}
\D t{Y_1}=3Y_1 \quad\text{and}\quad \D t{Y_2}=-Y_2\,.
\end{equation*}
Each of these differential equations have well-known \idx{exponential} solutions, respectively \(Y_1=c_1e^{3t}\) and \(Y_2=c_2e^{-t}\),  for every constants~\(c_1\) and~\(c_2\).
...
For example, suppose at time zero (\(t=0\)) the \idx{initial population} of \idx{female} \(y\)-animals is~\(22\) and the population of female \(z\)-animals is~\(9\).
...
Find (by hand) a \idx{general solution} to the system of differential equations \(\D tu=-2u+2v\)\,, \(\D tv=u-2v+w\)\,, and \(\D tw=2v-2w\)\,.
...
Find (by hand) a \idx{general solution} to the system of differential equations
\(\D ty=z\) and \(\D tz=-4y\)\,.
\begin{solution} 
...
In a real application the \idx{complex number}s of the \idx{general solution} to \autoref{eg:decis2t} are usually inconvenient.  
...
Write down three different matrices~\(P\) that will diagonalise the matrix~\(A\), and for each write down the corresponding \idx{diagonal matrix} \(D=P^{-1}AP\).
\begin{enumerate}
...
\begin{exercise} \label{ex:} 
From the given information, are each of the matrices \idx{diagonalisable}?  
Give reasons.
\begin{enumerate}
...
\begin{exercise} \label{ex:} 
For each of the following matrices, use \script\ to find the \idx{eigenvalue}s, their \idx{multiplicity}, and the \idx{dimension} of the \idx{eigenspace}s.  Give reasons (remember computational error).
%\begin{verbatim}
...
Recall the general complex solution that \autoref{eg:sprmas} derives for the \idx{oscillation}s of a mass on a spring.
Show that substituting \(c_1=(C_1-\i C_2)/2\) and \(c_2=(C_1+\i C_2)/2\) for real~\(C_1\) and~\(C_2\) results in the velocity~\(v(t)\) being expressed algebraically in purely real terms.
\end{exercise}
