...
\end{equation*}
In \script, construct this matrix with \index{bsxfun()@\texttt{bsxfun()}}\verb|U=bsxfun(@power,lambda,0:n-1).'|.
\end{enumerate}
\fi%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Consequently, and allowing \idx{complex eigenvalue}s, a real \idx{non-diagonalisable} matrix must be non-symmetric and must have at least one \idx{repeated eigenvalue}.
