...
\item For every \idx{symmetric matrix}~\(A\), the \bfidx{multiplicity} of an \idx{eigenvalue}~\(\lambda\) of~\(A\) is the \idx{dimension} of the corresponding \idx{eigenspace}~\(\EE_\lambda\) (\autoref{def:eigsymult}).

\item For every \(n\times n\) \idx{square matrix}~\(A\) (not just symmetric),
...
\item \autoref{pro:eeh} finds by hand \idx{eigenvalue}s and \idx{eigenvector}s of a (small) \idx{square matrix}~\(A\):
\begin{enumerate}
...
\item Let \(A\) be a \idx{square matrix} with \idx{eigenvalue}~\(\lambda\) and corresponding \idx{eigenvector}~\xv\ (\autoref{thm:ematpow}).
\begin{itemize}
...
\item For every real \idx{symmetric matrix}~\(A\), the \idx{eigenvalue}s of~\(A\) are all real (\autoref{thm:realeigsym}).

\item Let \(A\) be a real \idx{symmetric matrix}, then for every two distinct \idx{eigenvalue}s of~\(A\), any corresponding two \idx{eigenvector}s are \idx{orthogonal} (\autoref{thm:orthoevec}).

\item Every \(n\times n\) real \idx{symmetric matrix}~\(A\) has at most \(n\)~distinct \idx{eigenvalue}s (\autoref{thm:lenlam}).

\item Let \(A\) be an \(n\times n\) real \idx{symmetric matrix} with \svd\ \(A=\usv\).
...
\item For every real \idx{square matrix}~\(A\), 
