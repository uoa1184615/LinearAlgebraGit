...
=d_n\ev_n\,.
\end{eqnarray*}
By \cref{def:evecval}, each diagonal element~\(d_j\)  is an eigenvalue of the diagonal matrix, and the \idx{standard unit vector} \(\ev_j\)~is a corresponding eigenvector.
\end{example}
...
\begin{tabular}{@{}cc@{}}
\eRose{1}{-0.5}{-0.5}{-0.2} &
\parbox[b]{14em}{The plot on the left shows \idx{unit vector}s~\xv\  (blue), and for the matrix~\(A\) of \cref{eg:eig2pic2} the corresponding vectors~\(A\xv\) (red) adjoined. 
...
When an eigenvalue of a \idx{symmetric matrix} is duplicated (or triplicated) in the diagonal~\(D\) then there are many choices of eigenvectors that form an \idx{orthonormal basis} (\cref{def:orthobasis}) of the eigenspace (the same holds for \idx{singular vector}s of a duplicated singular value).
...
\cref{ch:ddm} extends the determinant to any \idx{size} matrix, and explores more useful properties, but for now this is the information we need on determinants.
