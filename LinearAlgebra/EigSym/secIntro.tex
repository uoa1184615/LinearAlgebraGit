...
\begin{activity}
Further, for the picture in \autoref{act:eig} that indicates~\xv\ is an eigenvector, is the corresponding eigenvalue~\(\lambda\):
...
\end{equation*}
\begin{solution} 
\autoref{eg:eigndiag} shows that this diagonal matrix has two distinct eigenvalues \(\lambda=-\frac13\) and \(\lambda=\frac32\)\,.
\begin{itemize}
...
\begin{table}
\caption{As well as the \script\ commands and operations listed in \cref{tbl:mtlbpre,tbl:mtlbbasics,tbl:mtlbops,tbl:mtlbmops,tbl:mtlbsvd}  we need the eigenvector function.\index{Matlab@\textsc{Matlab}|textbf}\index{Octave|textbf}} \label{tbl:mtlbeig}
\hrule
...
\begin{itemize}
\item Recall from previous study (\autoref{thm:2x2det}) that a \(2\times 2\) matrix \(A=\begin{bmat} a&b\\c&d \end{bmat}\) has \idx{determinant} \(\det A=|A|=ad-bc\)\,, and that \(A\)~is not \idx{invertible} \idx{iff} \(\det A=0\)\,.
\item Similarly, although not justified until \autoref{ch:ddm}, a \(3\times 3\) matrix \(A=\begin{bmat} a&b&c\\d&e&f\\g&h&i \end{bmat}\) has \idx{determinant} \(\det A=|A|=aei+bfg+cdh-ceg-afh-bdi\)\,, and \(A\)~is not \idx{invertible} iff \(\det A=0\)\,.
\end{itemize}
