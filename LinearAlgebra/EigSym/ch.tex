...
Recall (\cref{sec:amwm}) that a \idx{symmetric matrix}~\(A\) is a square matrix such that \(\tr A=A\)\,, that is, \(a_{ij}=a_{ji}\)\,.
...
\idx{random matrix} to its transpose with \index{randn()@\texttt{randn()}}\verb|A=randn(5); A=A+A'| (\cref{tbl:mtlbops}):
...
\paragraph{Solid modelling} Lean with a hand on a table\slash wall: the force changes depending upon the \idx{orientation} of the surface.  
