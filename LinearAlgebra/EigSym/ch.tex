...
\item Find its \svd\ via \verb|[U,S,V]=svd(A)|
\begin{verbatim}
U =
...
\end{enumerate}
Repeat and observe \(\uv_j=\pm \vv_j\) for all columns~\(j\).
\end{solution}
\end{example}
...
Because \(A=\usv\), post-multiplying by~\(V\) gives \(AV=US\tr VV=US\)\,, and then the \(j\)th~column of \(AV=US\) is \(A\vv_j=\sigma_j\uv_j\)\,.
...
\begin{comment}
Dynamics of two masses on a spring (relate to molecular vibrations), and seek solutions in~\(\cos(ft)\) for eigenvalue \(\lambda=f^2\).  As done for three masses in Example~\ref{eg:eig3vib}.  Say symmetry from equal and opposite, claim holds even for gigantic structures with millions of interacting components.
\end{comment}


\paragraph{Solid modelling} Lean with a hand on a table\slash wall: the force changes depending upon the orientation of the surface.  Similarly inside any solid: the internal forces\({}=A\nv\) where \(\nv\)~is the normal \idx{unit vector} to the internal `surface'.  
Matrix~\(A\) is always symmetric.  To know whether a material will break apart under pulling? or to crumble under compression?  we need to know where the extreme forces are.  They are found as solutions to \(A\nv=\lambda\nv\) where \nv~gives the direction and \(\lambda\)~the strength of the force.
