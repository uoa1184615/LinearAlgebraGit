...
To establish a \idx{contradiction}: assume the eigenvalue~\(\lambda\) is complex valued, and so correspondingly is the eigenvector~\xv.
...
The previous theorem establishes there are at most \(n\)~distinct \idx{eigenvalue}s (for symmetric matrices but it is true more generally).  
...
Second, for the singular values \(\sigma_j>0\) use a form of \idx{induction} allied with a \idx{contradiction} to prove \(\uv_j=\pm \vv_j\).
...
The challenge of this subsection is to choose new coordinates so that the quadratic equation~\eqref{eq:gqe} becomes one of these recognised canonical forms.
\begin{description}
