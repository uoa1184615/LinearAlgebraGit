...
\item The \(9\times9\) matrix of the Sierpinski network in Example~\ref{eg:sier2eig} is not invertible as it has zero among its five eigenvalues.
...
In contrast, in directions which are not eigenvectors, the relationship between~\(A\xv\) and~\(A^{-1}\xv\) is obscure.
\end{itemize}
\end{example}
...
\begin{enumerate}
\item\label{thm:ematpow:i} For any positive integer~\(n\), \(\lambda^n\) is an \idx{eigenvalue} of~\(A^n\) with corresponding \idx{eigenvector}~\xv.
...
\item\label{thm:ematpow:iii} If \(A\) is \idx{invertible}, then for any integer~\(n\), \(\lambda^n\) is an \idx{eigenvalue} of~\(A^n\) with corresponding \idx{eigenvector}~\xv.
\end{enumerate}
\end{theorem}
\begin{proof} 
...
\begin{activity}
Given that \(-3\) and~\(2\) are eigenvalues of the matrix \(A=\begin{bmatrix} 1&2\\2&-2 \end{bmatrix}\), which of the following matrices has an eigenvalue of~\(8\)?
\partswidth=5em
\begin{parts}
\item \(A^{-2}\)
\item \(A^{-1}\)
...
But the algebra still works with \idx{complex eigenvalue}s (Chapter~\ref{ch:gee}).
...
Consequently, the proof of the next Theorem~\ref{thm:realeigsym} needs to use some \idx{complex numbers} and some properties of complex numbers.
...
Such properties of complex numbers and operations also hold for complex valued vectors, complex valued matrices, and arithmetic operations with complex matrices and vectors.
...
Let \(A\) be a real \idx{symmetric matrix}, then the \idx{eigenvalue}s of~\(A\) are all real.
\end{theorem}
...
Hence (Definition~\ref{def:orthovec}) the two eigenvectors are orthogonal.
\end{proof}
...
Let \(A\) be an \(n\times n\) real \idx{symmetric matrix}, then \(A\)~has at most \(n\)~distinct \idx{eigenvalue}s.
\end{theorem}
\begin{proof} 
...
The previous theorem establishes there are at most \(n\)~distinct \idx{eigenvalue}s (here for symmetric matrices but Theorem~\ref{thm:geecp} establishes it is true for general matrices).  
...
\begin{aside} This proof modifies parts of the proof of the \svd\ Theorem~\ref{thm:svd} to the specific case of a symmetric matrix.
\end{aside}
\begin{proof} 
...
Recall the \idx{induction} proof for the \svd\ Theorem~\ref{thm:svd}  (section~\ref{sec:dpsvdt}).  
...
Let \(A\) be real \idx{square matrix}.  
Then matrix~\(A\) is symmetric iff it is \idx{orthogonally diagonalisable}.
\end{theorem}
...
Example~\ref{eg:cuceh} illustrated an ellipse and a hyperbola.  These curves are examples of the so-called \bfidx{conic section}s which arise as solutions of the \idx{quadratic equation} in two variables, say~\(x\) and~\(y\),
\begin{equation}
...
\begin{example}[rotate coordinates] \label{eg:}
...
Relate to the eigenvalues of Example~\ref{eg:2x2orthevec}.
\begin{solution} 
...
\begin{theorem}[extreme values] \label{thm:optqform}
...
\begin{proof} 
Change to an orthogonal coordinate system~\yv\ that diagonalises the matrix~\(A\) (Theorem~\ref{thm:patqform}): say coordinates \(\yv=V\xv\) for orthogonal matrix~\(V\) whose columns are orthogonal eigenvectors of~\(A\) in order so that \(D=\tr VAV=\diag(\hlist\lambda n)\).
Then the quadratic form 
\begin{equation*}
\tr\xv A\xv=\tr{(V\yv)} A(V\yv)=\tr\yv \tr VAV\yv=\tr\yv D\yv\,.
\end{equation*}
...
What is the maximum of the quadratic form \(\tr\xv A\xv\) over unit vectors~\xv?
\partswidth=5em
\begin{parts}
\item \(-14\)
\item \(0\)
...
\item \(14\)
\end{parts}
Further, what is the minimum of the quadratic form \(\tr\xv A\xv\) over unit vectors~\xv?
\end{activity}
