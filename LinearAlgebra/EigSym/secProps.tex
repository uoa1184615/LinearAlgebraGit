...
Symmetric matrices have the beautifully useful properties of always having real eigenvalues and always having orthogonal eigenvectors.
...
\cref{eg:ccevals,eg:ltas}). 
That real symmetric matrices always have real eigenvalues (such as in all matrices of \cref{eg:2x2sympow,eg:3x3sympow}) is a special property that marvellously often reflects the physical reality of many applications.
...
Recall that any complex number \(z=a+b\i\) has a \idx{complex conjugate} \(\bar z=a-b\i\) (denoted by the overbar, and similarly for vectors), and that a complex number equals its conjugate only if it is real valued (the imaginary part is zero).
...
In general the nonzero eigenvector~\xv\ is complex valued, say 
\begin{eqnarray*}
...
%To create a \idx{contradiction} that establishes the theorem: assume the eigenvalue~\(\lambda\) is not real valued (that is, it is complex with a non-zero imaginary part).
%Since \(A\xv=\lambda\xv\), the eigenvector~\xv\ must have a non-zero imaginary part.
%Second, the complex conjugate~\(\bar\lambda\) must be an eigenvalue of~\(A\), corresponding to an eigenvector which is the complex conjugate~\(\bar\xv\).
%If the eigenvalue~\(\lambda\) is real, then its complex conjugate \(\bar\lambda=\lambda\) and so is the same eigenvalue---similarly for~\(\bar\xv\).
%To see that~\(\bar\lambda\) is an eigenvalue just take the complex conjugate of both sides of \(A\xv=\lambda\xv\)\,:
%\begin{equation*}
...
That is, the eigenvalue~\(\lambda\) must be real.
\end{proof}
...
Hence (\cref{def:orthovec}) the two eigenvectors are orthogonal.
\end{proof}
...
\end{theorem}
\begin{proof} 
%Invoke the \idx{pigeonhole principle} and 
...
\begin{exercise}  
For each of the following symmetric matrices: from a hand derivation of the \idx{characteristic equation} (defined in \cref{pro:eeh}), determine whether each matrix has a zero \idx{eigenvalue} or not, and hence determine whether it is \idx{invertible} or not.
\begin{Parts}
...
Confirm that each column of~\(V\) is an \idx{eigenvector} of the matrix (that is, proportional to what the exercise found) and the corresponding \idx{singular value} is the magnitude of the corresponding \idx{eigenvalue} (\cref{thm:smevec}).
