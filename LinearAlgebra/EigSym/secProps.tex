...
\index{eigenvector|(}
Recall that \autoref{sec:im} introduced the \idx{inverse} of a matrix (\autoref{def:invertible}).
...
\item The \(9\times9\) matrix of the \idx{Sierpinski network} in \autoref{eg:sier2eig} is not invertible as it has zero among its five eigenvalues.
...
\item The \(2\times2\) \idx{non-symmetric matrix} of \autoref{eg:ccevals} is invertible because zero is not among its eigenvalues of \(\lambda=\pm \i\)\,.
Indeed, the matrix
\begin{equation*}
...
\end{equation*}
is diagonal so its eigenvalues are the \idx{diagonal elements} (\autoref{eg:eigndiag}), namely the only eigenvalue is~\(-1\).
...
Its \idx{characteristic equation} (defined in \autoref{pro:eeh}) is
\begin{equation*}
\det(A^{-1}-\lambda I)
...
\index{orthogonally diagonalisable|(}
\index{diagonalisable!orthogonally|(}
...
The contradiction is to assume a \idx{complex valued} eigenvalue exists, and then prove it cannot.
Consequently, the proof of the next \autoref{thm:realeigsym} needs to use some \idx{complex number}s and some properties of complex numbers.
...
the second part are symmetric and from the marginal illustration their eigenvectors are proportional to \((1,1)\) and~\((-1,1)\) which are orthogonal directions---they are at \idx{right-angles} in the illustration.

\begin{example} \label{eg:}
Recall \autoref{eg:3x3hande} found the \(3\times3\) \idx{symmetric matrix}
\begin{equation*}
...
Confirm the eigenvectors are orthogonal for a \idx{symmetric matrix}.
\begin{parts}
...
The previous theorem establishes there are at most \(n\)~\idx{distinct eigenvalues} (here for symmetric matrices, but \autoref{thm:geecp} establishes it is true for general matrices).  
...
A real \idx{square matrix}~\(A\) is \bfidx{orthogonally diagonalisable} if there exists an \idx{orthogonal matrix}~\(V\) and a \idx{diagonal matrix}~\(D\) such that \(\tr VAV=D\), equivalently \(AV=VD\), equivalently \(A=VD\tr V\) is a \idx{factorisation} of~\(A\).
\end{definition}
...
\begin{example} \label{eg:}
\begin{enumerate}
\item\label{eg:2x2orthdiag} Recall from \autoref{eg:2x2orthevec} that the \idx{symmetric matrix} \(A=\begin{bmatrix} 1&\frac32\\\frac32&-3 \end{bmatrix}\) has eigenvalues \(\lambda=-\tfrac72,\tfrac32\) with corresponding orthogonal eigenvectors \((1,-3)\) and~\((3,1)\).
...
\begin{solution} Among several possibilities are the following.
\begin{parts}
...
\autoref{eg:cuceh} illustrated an ellipse and a \idx{hyperbola}.  These curves are examples of the so-called \bfidx{conic section}s which arise as solutions of the \idx{quadratic equation} in two variables, say~\(x\) and~\(y\),
\begin{equation}
...
The challenge of this subsection is to choose good new coordinates so that a given quadratic equation~\eqref{eq:gqe} becomes one of these recognised \idx{canonical form}s.
\begin{description}
...
\autoref{eg:cuceh} implicitly has two steps: first, we decide upon an orientation for the \idx{coordinate axes}; second, we decides that the \idx{coordinate system} should be `centred' in the picture.
...
The vectors in the plane will be written as the \idx{linear combination} \(\xv=\vv_1x'+\vv_2y'\).
...
\item \autoref{eg:csrc2} found the \idx{hyperbola} satisfying equation \(x^2+3xy-3y^2-\tfrac12=0\)\,.
This equation may be written in terms of a quadratic form as \(\tr\xv A\xv-\tfrac12=0\) for vector \(\xv=(x,y)\) and \idx{symmetric matrix} \(A=\begin{bmatrix} 1&\tfrac32\\\tfrac32&-3 \end{bmatrix}\).

\item \autoref{eg:cstrrc2} found the \idx{ellipse} satisfying the equation
...
For every \idx{quadratic form}, there exists an \idx{orthogonal} \idx{coordinate system} that diagonalises the quadratic form.
...
Relate to the \idx{eigenvalue}s of \autoref{eg:2x2orthevec}.
\begin{solution} 
...
\begin{activity}
Recall \autoref{eg:3x3hande} found that the \(3\times3\) \idx{symmetric matrix}
\begin{equation*}
...
By assessing whether there are any zero eigenvalues, estimate if the matrix~\(A\) is \idx{invertible} or not.
...
\begin{exercise} \label{ex:} 
For each of the following symmetric matrices: from a hand derivation of the \idx{characteristic equation} (defined in \autoref{pro:eeh}), determine whether each matrix has a zero \idx{eigenvalue} or not, and hence determine whether it is \idx{invertible} or not.
\begin{parts}
...
For each of the following matrices, find by hand the \idx{eigenvalue}s and \idx{eigenvector}s.  
Using these eigenvectors, confirm that the eigenvalues of the matrix squared are the square of its eigenvalues.
If the matrix has an \idx{inverse}, what are the eigenvalues of the inverse?
%\begin{verbatim}
%format rat,n=3
%for i=1:10^n
...
By roughly estimating \idx{eigenvalue}s and \idx{eigenvector}s by eye, identify the pairs of plots corresponding to each matrix and its inverse.
\begin{parts}
...
\begin{exercise} \label{ex:} 
For the symmetric matrices of \autoref{ex:smoevs}, confirm that \idx{eigenvector}s corresponding to \idx{distinct eigenvalues} are orthogonal (\autoref{thm:orthoevec}).  Show your working.
\end{exercise}
...
Confirm that each column of~\(V\) is an \idx{eigenvector} of the matrix (that is, proportional to what the exercise found) and the corresponding \idx{singular value} is the magnitude of the corresponding \idx{eigenvalue} (\autoref{thm:smevec}).
...
Confirm that each column of~\(V\) is \emph{not} an \idx{eigenvector} of the matrix, and the \idx{singular value}s do \emph{not} appear closely related to the \idx{eigenvalue}s.
Show and discuss your working.
\end{exercise}
...
for any integer~\(n\), \(\lambda^n\) is an \idx{eigenvalue} 
...
\begin{exercise} \label{ex:} 
For each of the following matrices, give reasons as to whether the matrix is \idx{orthogonally diagonalisable}, and if it is then find an \idx{orthogonal matrix}~\(V\) that does so and the corresponding \idx{diagonal matrix}~\(D\).
...
\begin{exercise} \label{ex:} 
Let matrix~\(A\) be invertible and \idx{orthogonally diagonalisable}.  
Show that the \idx{inverse}~\(A^{-1}\) is \idx{orthogonally diagonalisable}.
\end{exercise}
...
\begin{exercise} \label{ex:} 
Suppose matrices~\(A\) and~\(B\) are \idx{orthogonally diagonalisable} by the same \idx{orthogonal matrix}~\(V\).  
...
\begin{exercise} \label{ex:} 
For each of the following matrices, say~\(A\), consider the \idx{quadratic form} \(q(\xv)=\tr\xv A\xv\).
...
\begin{exercise} \label{ex:} 
For a \idx{symmetric matrix}, discuss the similarities and differences between the \svd\ and the diagonalisation factorisation, the \idx{singular value}s and the \idx{eigenvalue}s, and the \idx{singular vector}s and the \idx{eigenvector}s.
\end{exercise}
