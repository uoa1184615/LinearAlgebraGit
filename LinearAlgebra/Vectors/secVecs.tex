...
In the eighteenth century, astronomers needed to describe both the position and the velocity of the \idx{planets}. 
...
Given two points in space, say~\(A\) and~\(B\), the displacement vector~\(\ovect{AB}\) is the directed line segment from the point~\(A\) to the point~\(B\)---as illustrated by the two displacement vectors \(\ovect{AB}\) and~\(\ovect{CD}\) to the right.
...
\end{tikzpicture}}%
\needlines{5}
\begin{wrapfigure}[5]r{0pt} \temp0 \end{wrapfigure}%
\begin{example}[position vector] \label{eg:posvec} 
The next important class of vectors are the \bfidx{position vector}s.
Given some chosen fixed origin in space, usually denoted~\(O\), then \(\ovect{OA}\)~is the \idx{position vector} of the point~\(A\).
This picture illustrates the \idx{position vector}s of four points in the plane (\(A,B,C,D\)), from the given origin~\(O\).
...
\begin{tabular}c \temp0 \\ \temp1 \end{tabular}
\end{wrapfigure}%
...
Putting these together forms the velocity vector \((831.5,344.4,0)\) in~km/hr \text{in space.}
...
\begin{itemize}
\item 
\begin{figbox}[\vspace{2ex}]{\begin{tikzpicture} 
\begin{axis}[footnotesize,font=\footnotesize
...
\end{axis}
\end{tikzpicture}}%
In the two dimensions of the plane the \idx{standard coordinate system} has two \idx{coordinate axes}, 
...
\item \begin{figbox}[\vspace{5ex}]{\begin{tikzpicture} 
...
\end{axis}
\end{tikzpicture}}%
In the three dimensions of space the \idx{standard coordinate system} has three \idx{coordinate axes}, 
...
\begin{wrapfigure}r{0pt} \begin{tabular}{@{}cc@{}}
% AJR's photo, taken in Wales
\includegraphics[width=4.5em]{Vectors/RobertRecorde}&
\parbox[b]{16em}{\index{Recorde, Robert}\rm
...
\end{tabular} \end{wrapfigure}\strut{}%
...
Be aware of this different notation in reading \text{other books.}
...
The next example illustrates another use \text{of vectors.}
...
These \idx{word vector}s are in~\(\RR^5\) as there are five components in each vector~\wv.
Then
\begin{enumerate}
...
The ability to number-crunch huge amounts of words for the first time has led to entirely new ways of thinking about words and their relationship to \text{one another.}
...
This set of relationships can be thought of as a multidimensional vector that describes how the word Olympics is used within a language, which itself can be thought of as a \text{vector space.}
...
\begin{activity}
Given \idx{word vector}s \(\sloppy\wv=(N_{\text{cat}}, N_{\text{dog}}, N_{\text{mat}}, N_{\text{sat}}, N_{\text{scratched}})\) as in \cref{eg:deflsv},
...
define the \bfidx{length}, or \bfidx{magnitude}, of a vector~\vv\  to be the real number~(\(\geq0\))
...
Remember that the term \emidx{size} denotes the number of components in a vector (\cref{def:vecs}) and so ``size'' should not be used for the length\slash magnitude of a vector.
...
\begin{example} 
Write down three different vectors, all three with the same number of components, for each of the following cases: (a)~of \idx{length}~\(5\), (b)~of length~\(3\), and (c)~of length~\(-2\).
\begin{solution} 
\begin{enumerate}
...
Confirm that your calculated lengths reasonably approximate your measured lengths.
\end{exercise}
