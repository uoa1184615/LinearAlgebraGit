...
Quantities that have both the properties of a magnitude and a direction are called \bfidx{vectors} (from the Latin for \emph{carrier}).
...
To be able to manipulate vectors we describe them with numbers from a \idx{coordinate system}.
...
Again, to be able to manipulate such vectors we describe them with numbers from a \idx{coordinate system}.
\marginpar{\temp{1}}
...
Of course you recognise that the ordered pairs of numbers in the position vectors are exactly the coordinates of each of the specified end-points. 
\end{example}
...
\end{axis}
\end{tikzpicture}}%
But West is the opposite direction to East, so if we choose to write East as positive, then West must be negative.
...
Then the North and West speeds are \(100\cos(\tfrac\pi4)=70.71\)\,m/s.
...
\begin{axis}[footnotesize,font=\footnotesize
  ,axis equal,  ,axis lines=middle, grid
  ,ymin=-1.9,xmin=-1.9,xmax=3.9,ymax=3.6
  ] 
...
Question: why do we need vectors with \(n\)~components, in~\(\RR^n\), when the world around is is only three dimensional?
...
Given any \idx{vector} \(\vv=(\hlist vn)\) in~\(\RR^n\),
...
\end{equation*}
%A vector of length zero is called the \bfidx{zero vector} and denoted~\ov.
...
\newcommand{\mytemp}{\begin{tikzpicture} 
\begin{axis}[small,font=\small%footnotesize,font=\footnotesize
...
Being consistent about the order of words, represent each of the eight titles by a \idx{word vector} in~\(\RR^7\).
