...
\item \index{+,-,*@\texttt{+,-,*}|textbf}\verb|+,-,*| is vector\slash scalar addition, subtraction, and multiplication, but only provided the sizes of vectors are the same.
...
Use the \script\ command \index{norm()@\texttt{norm()}}\verb|norm()| to compute the \idx{length}\slash \idx{magnitude} of the following vectors (\autoref{def:veclen}).
\begin{enumerate}
\item \((2,-1)\)
\begin{solution} 
...
As indicated by \autoref{tbl:mtlbpre} the numbers with \idx{brackets} separated by semi-colons forms a vector, and the \verb|=|~character assigns the result to variable for subsequent use.
...
Use \script\ to compute the \idx{length} of each of the following vectors (the first five have integer lengths).
\begin{parts}[6em]
