...
The aim is to develop some basic confidence with \script\ before later using it to save you considerable time in longer tasks.
...
\item \script\ potentially uses \idx{complex number}s~(\index{C@\CC}\CC), but mostly we stay within real numbers~(\index{R@\RR}\RR).
...
As indicated by \cref{tbl:mtlbpre} the numbers with \idx{brackets} separated by semi-colons forms a vector, and the \verb|=|~character assigns the result to a variable for subsequent use.
