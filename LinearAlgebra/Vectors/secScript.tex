...
The aim is to develop some basic confidence with \script\ before later using it to save you considerable time in \text{longer tasks.}
...
\footnote{If desired, `\idx{computer algebra}' software provides us with an arbitrary level of \idx{precision}, even exact.
...
\item \script\ potentially use \idx{complex number}s~(\index{C@\CC}\CC), but mostly we stay within real numbers~(\index{R@\RR}\RR).

\item Each \script\ command is usually typed on one line by itself.

\item \index{[\ldots]@\texttt{[\ldots]}|textbf}\verb|[ . ; . ; . ]| where each dot denotes a number, forms vectors in~\(\RR^3\) (or use newlines instead of the semicolons).  
Use \(n\)~numbers separated by semicolons for vectors in~\(\RR^n\).

\item \index{=@\texttt{=}|textbf}\verb|=| assigns the result of the expression to the right of the~\verb|=| to the variable name on the left.
If the result of an expression is not explicitly assigned to a variable, then by default it is assigned to the variable~\index{ans@\texttt{ans}|textbf}\verb|ans|.
%(denoted by ``\index{ans@\texttt{ans}}\verb|ans =|'' in \script).

\item Variable names are alphanumeric starting with a letter.

\item \index{size()@\texttt{size()}|textbf}\verb|size(v)| returns the number of \idx{components} of the vector (\cref{def:vecs}): if the vector~\vv\ is in~\(\RR^m\), then \verb|size(v)| returns \(\begin{bmatrix} m&1 \end{bmatrix}\).

\item \index{norm()@\texttt{norm()}|textbf}\verb|norm(v)| computes the \idx{length}\slash \idx{magnitude} of the vector~\verb|v| (\cref{def:veclen}).

\item \index{+,-,*@\texttt{+,-,*}|textbf}\verb|+,-,*| is vector\slash scalar addition, subtraction, and scalar multiplication, but only provided the sizes of vectors are the same.
Parentheses \index{()@\texttt{()}}\verb|()| control the order of operations.

\item \index{/@\texttt{/}|textbf}\verb|/x| divides a vector\slash scalar by a scalar~\(x\).
However, be warned that \verb|/v| for a vector~\vv\ typically gives a strange result as \script\ interprets it to mean you want to approximately solve some \idx{linear equation}.

\item \index{^@\texttt{\^}|textbf}\verb|x^y| for scalars~\(x\) and~\(y\) computes~\(x^y\).

\item \index{dot()@\texttt{dot()}|textbf}\verb|dot(u,v)| computes the \idx{dot product} of vectors~\verb|u| and~\verb|v| (\cref{def:dotprod})---if they have the same size.

\item \index{acos()@\texttt{acos()}|textbf}\verb|acos(q)| computes the \idx{arc-cos}, the \idx{inverse cosine}, of the scalar~\verb|q| in radians.  
To find the angle in degrees use \verb|acos(q)*180/pi| (\script\ knows \(\verb|pi|=\pi=3.14159\cdots\)).\index{pi@\texttt{pi}}

\item \index{quit@\texttt{quit}|textbf}\verb|quit| terminates the \script\ session.
\end{itemize}
\end{minipage}
\hrule
\end{table}
...
\begin{example} 
Use the \script\ command called \index{norm()@\texttt{norm()}}\verb|norm()| to compute the \idx{length}\slash \idx{magnitude} of the following vectors (\cref{def:veclen}).
...
As indicated by \cref{tbl:mtlbpre} the numbers with \idx{brackets} separated by semicolons form a vector, and the \verb|=|~character assigns the result to a variable for \text{subsequent use.}
...
\marginajrbox%
You may scan such \idx{qr-code}s with your favourite app\footnote{At the time of writing, qr-code scanning applications for smart-phones include \emph{WaspScan} and \emph{QRReader}---but I have no expertise to assess their quality.}, and then copy and paste the code direct into a \script\ client.
Alternatively, if reading an electronic version of this book, then you may copy and paste the commands (although often the \idx{quote} character~\verb|'| needs correcting).
...
\begin{example} 
Use \script\ operators \index{+,-,*@\texttt{+,-,*}}\verb|+,-,*| to compute the value of the expressions \(\uv+\vv\), \(\uv-\vv\), \(3\uv\) for vectors \(\uv=(-4.1,1.7,4.1)\) and \(\vv=(2.9,0.9,-2.4)\) (\cref{def:vecops}).
...
\begin{example} 
Use \script\ to confirm the \idx{commutative law} (\cref{thm:vecopsa}) \(\uv+\vv=\vv+\uv\) for vectors
...
(In such a message, ``\index{nonconformant 
arguments@\texttt{nonconformant arguments}}\texttt{nonconformant arguments}'' means that the vectors are of a wrong size.)
\begin{verbatim}
...
\end{tabular}}{\tt\begin{tabular}r
    ...
    4.3000\\
   -7.1000
\end{tabular}}{\tt\begin{tabular}{p{8em}}
Error using~* 
...
For the vectors of the previous \cref{acvecsub}:
 which is the result of typing the command \verb|2*u|? %d
 which is the result of typing the command \verb|u*v|? %c
\end{activity}
...
Verify the \idx{distributive law} for the \idx{dot product} \((\uv+\vv)\cdot\wv=\uv\cdot\wv+\vv\cdot\wv\) (\cref{thm:dotopsf}) for vectors
...
\begin{exercise}  
Use \script\ to compute the \idx{length} of each of the following vectors.
\begin{Parts}[4]
...
\begin{exercise}  
Use \script\ to find the angles between all pairs of vectors in each of the following groups.
...
\begin{enumerate}
\item Explain the differences between \index{size()@\texttt{size()}}\verb|size(v)| and \index{norm()@\texttt{norm()}}\verb|norm(v)|?
