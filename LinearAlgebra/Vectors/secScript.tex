...
\item \index{[\ldots]@\texttt{[\ldots]}|textbf}\verb|[ . ; . ; . ]| where each dot denotes a number, forms vectors in~\(\RR^3\) (or use newlines instead of the semi-colons).  
...
As indicated by \cref{tbl:mtlbpre} the numbers with \idx{brackets} separated by semi-colons forms a vector, and the \verb|=|~character assigns the result to a variable for \text{subsequent use.}
...
arguments@\texttt{nonconformant arguments}}\texttt{nonconformant arguments}'' means the vectors are of a wrong size.
\begin{verbatim}
