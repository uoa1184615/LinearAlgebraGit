...
\item \index{size()@\texttt{size()}|textbf}\verb|size(v)| returns the number of \idx{components} of the vector (\autoref{def:vecs}): if the vector~\vv\ is in~\(\RR^m\), then \verb|size(v)| returns \(\begin{bmatrix} m&1 \end{bmatrix}\).

\item \index{norm()@\texttt{norm()}|textbf}\verb|norm(v)| computes the \idx{length}\slash \idx{magnitude} of the vector~\verb|v| (\autoref{def:veclen}).
...
\item \index{dot()@\texttt{dot()}|textbf}\verb|dot(u,v)| computes the \idx{dot product} of vectors~\verb|u| and~\verb|v| (\autoref{def:dotprod})---if they have the same size.
...
\begin{example} 
Use the \script\ command called \index{norm()@\texttt{norm()}}\verb|norm()| to compute the \idx{length}\slash \idx{magnitude} of the following vectors (\autoref{def:veclen}).
...
As indicated by \autoref{tbl:mtlbpre} the numbers with \idx{brackets} separated by semi-colons forms a vector, and the \verb|=|~character assigns the result to a variable for subsequent use.
...
\begin{example} 
Use \script\ to confirm the \idx{commutative law} (\autoref{thm:vecopsa}) \(\uv+\vv=\vv+\uv\) for vectors
...
\begin{reduce}
\begin{example} 
Verify the \idx{distributive law} for the \idx{dot product} \((\uv+\vv)\cdot\wv=\uv\cdot\wv+\vv\cdot\wv\) (\autoref{thm:dotopsf}) for vectors
