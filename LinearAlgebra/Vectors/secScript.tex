...
\item \script[2]\ is free software, that functionally is largely the same as \script[1], and downloadable over the internet.\footnote{\url{https://www.gnu.org/software/octave/}} 
...
\begin{table}
\caption{Use \script\ to help compute vector results with the following basics.  This and subsequent tables throughout the book summarise the main \script\ information.
\index{Matlab|textbf}\index{Octave|textbf}} \label{tbl:mtlbpre}
\hrule
...
\item \script\ potentially uses complex numbers~(\CC), but mostly we stay with real numbers~(\RR).
...
Use more numbers separated by semi-colons for vectors in other~\(\RR^n\).

\item \index{=@\texttt{=}}\verb|=| assigns the value of the expression on the right to the variable name on the left.

If the computation of an expression is not assigned to anything explicitly, then by default it is assigned to the variable~\verb|ans| (as  denoted by ``\index{ans@\texttt{ans}}\verb|ans =|'' in \script).
...
Parentheses \index{()@\texttt{()}}\verb|()| control the order of the operations.

\item \index{/@\texttt{/}}\verb|/x| divides by a scalar~\(x\).
...
\item \index{dot()@\texttt{dot()}}\verb|dot(u,v)| computes the \idx{dot product} of the vectors~\verb|u| and~\verb|v| (Definition~\ref{def:dotprod}), provided they have the same size.
...
Although here the saving in typing is negligible, later one can save considerable typing
\end{solution}
