...
For any two non-zero vectors~\uv\ and~\vv\ in~\(\RR^n\),  the \bfidx{angle}~\(\theta\) between the vectors is determined by 
\begin{equation*}
...
\begin{table}
\caption{when a \idx{cosine} is one of these special values, then we know the corresponding \idx{angle} exactly.  
In other cases we usually use a calculator (\(\arccos\) or \(\texttt{cos}^{-1}\)) or computer \index{acos()@\texttt{acos()}}(\texttt{acos()}) to compute the angle numerically.}
\label{tbl:cosines}
...
\begin{center}
\foreach \q in {50,55} {\begin{tikzpicture} 
...
Consider the general case when a vector force~\Fv\ acts on a body which moves a displacement~\dv.
\marginpar{\mytemp0}%
Then the work done by the force on the body is the length of the displacement times the component of the force in the direction of the displacement---the component of the force at right-angles to the displacement does no work.
...
Let \uv, \vv\ and~\wv\ be any vectors in~\(\RR^n\), and let \(a\)~be any \idx{scalar}.
...
\bfidx{orthogonal}\marginpar{The term `orthogonal' derives from the Greek for `right-angled'.} (or \bfidx{perpendicular})
...
\begin{example} \label{eg:}
First find the equation of the line that is perpendicular to the vector~\((2,3)\) and that passes through the origin.
Second, find is the equation of the line that passes through the point~\((4,1)\), instead of the origin?
\begin{solution} 
Recall that vectors at right-angles have a zero dot product (section~\ref{sec:ovra}).
Thus the position vector~\xv\ of any point in the line satisfies the dot product \(\xv\cdot(2,3)=0\)\,.
...
When the line goes through \((4,1)\), then it is the displacement \(\xv-(4,1)\) that must be orthogonal to~\((2,3)\), as illustrated.
...
\begingroup\newcommand{\mytmp}[1]{%
\foreach \q in {20,25}{\begin{tikzpicture} 
...
As illustrated below, that means to find all points~\(X\) such that~\(\ovect{PX}\) is orthogonal to~\nv.
\begin{center}\mytmp0\end{center}
Denote the position vector of~\(P\) by~\(\pv=(x_0,y_0,z_0)\),  the position vector of~\(X\) by \(\xv=(x,y,z)\), and let the normal vector be \(\nv=(a,b,c)\).
...
\begin{example} \label{eg:}
Find an \idx{equation of the plane} through point \(P=(1,1,2)\) and which is normal to \(\nv=(1,-1,3)\).
...
\begin{example} \label{eg:}
Write down a \idx{normal vector} to the following planes:
\begin{parts}
\item \(3x-6y+z=4\)\,;
...
\newcommand{\mytemp}[1]{%
\foreach \q in {10,15} {\begin{tikzpicture}
\begin{axis}[footnotesize,font=\footnotesize,height=5cm
