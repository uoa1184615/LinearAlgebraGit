...
The upcoming \cref{thm:triscal} establishes that this connection between dot products and lengths is no coincidence.
...
Let vectors \(\uv=(3,4)\) and \(\vv=(2,1)\) and consider the line given parametrically \index{parametric equation}(\cref{def:parlin}) as the \idx{position vector}s \(\xv=\uv+t\vv=(3+2t,4+t)\) for scalar parameter~\(t\)---illustrated in the margin.
\marginpar{\begin{tikzpicture} 
\begin{axis}[footnotesize,font=\footnotesize
