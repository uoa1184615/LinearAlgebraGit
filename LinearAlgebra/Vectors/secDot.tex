...
The interest in this \cref{eg:ezyang} is the number nine on the right-hand side of \(|\uv||\vv|\cos\theta=9\)\,.  
...
However, this formula applies only for one dimensional force and displacement, the case when the force and the displacement are all in the same direction.
...
\end{tikzpicture}}}
\needlines{12}
\begin{wrapfigure}[7]r{0pt}\mytemp0\end{wrapfigure}
\begin{example} 
...
To do so the \idx{parametric equation}s just uses two vectors with the corresponding number \text{of components.}
...
\begin{enumerate}
\item When using the dot product to determine the angle between a pair of vectors we only discuss angles between~\(0^\circ\) and~\(180^\circ\) (between~\(0\) and~\(\pi\) radians).  Why do we not discuss larger angles, such as~\(246^\circ\) or~\(315^\circ\)? nor \text{negative angles?}
