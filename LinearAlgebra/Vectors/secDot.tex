...
\end{aside}%
This section introduces the so-called dot product of two vectors that, among other attributes, gives a valuable way to determine the angle between the two vectors.
...
Consider the body-centered cube of atoms drawn in stereo below with the center of the cube at the origin: what is the angle between the center atom and any two adjacent corner atoms?
\begin{center}
\qview{62}{67} {
\begin{tikzpicture} 
\begin{axis}[footnotesize,font=\footnotesize,view={\q}{30}
...
\end{enumerate}
Use the \idx{angle} between these three \idx{word vector}s to characterise the similarity of the sentences: a small angle means the sentences are somehow close; a large angle means the sentences are disparate.
\begin{solution} 
...
The \idx{Cauchy--Schwarz inequality} inequality is one point of distinction between this `vector multiplication' and \idx{scalar multiplication}: for scalars \(|ab|=|a||b|\), but the dot product of vectors is typically less, \(|\uv\cdot\vv|\leq|\uv||\vv|\).
...
For vectors at \idx{right-angles} \(\theta=90^\circ\) and so \(\cos\theta=0\) and hence nonzero vectors are at right-angles only when the dot product \(\uv\cdot\vv=0\)\,.
...
\item \(0=\uv\cdot\ev_2=(\hlist un)\cdot(1,0,\ldots,0)=0+u_2+0+\cdots+0=u_2\), and so the second component must be zero; 
\item and so on to
...
\begin{activity}
What is an equation of the line through the point~\((4,2)\) and that is a \idx{right-angles} to the vector~\((1,3)\)?
...
\begin{exercise} \label{ex:} 
Recall \autoref{ex:8siambks} found \idx{word vector}s in~\(\RR^7\) for the titles of eight books that The Society of Industrial and Applied Mathematics (\textsc{siam}) reviewed recently.
...
\item What properties of the dot product differ from that of the multiplication of numbers?  
...
\item Given that \(\xv=\pv+t\dv\) parametrises a line, and that \(\xv=\pv+s\cv+t\dv\) parametrises a plane, what would \(\xv=\pv+r\bv+s\cv+t\dv\) describe? why? are there any provisos?
