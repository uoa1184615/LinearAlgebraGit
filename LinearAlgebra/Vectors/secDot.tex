...
This section introduces one such multiplication, the so-called dot product of two vectors that, among other attributes, gives a valuable way to determine the angle between \text{two vectors.}

{%begingroup
\needlines{11}
\def\vecopsHook{\node[above] at (axis cs:0,0) {$\qquad\theta$};}
\begin{wrapfigure}r{0pt}\begin{tabular}c
\vecops07{-1}25\\\vecops37{-1}25
...
Recall the \idx{cosine rule} for triangles that
\begin{equation*}
...
This equality suggests that the sum of products of corresponding components of~\uv\ and~\vv\ is closely connected to the angle between \text{the vectors.}
}%endgroup
...
But for the vector dot product, the dot must not be omitted: ``\(\uv\vv\)'' \text{is meaningless.}
...
Let's prove this theorem after \text{some examples.}
...
In other cases, we usually use a calculator (\texttt{arccos} or \(\texttt{cos}^{-1}\)) or computer (\texttt{acos()}) to compute the \text{angle numerically.}}
\label{tbl:cosines}
...
Then a calculator (or \script, see \cref{sec:umovc}) gives the angle to be \(\theta =\arccos(0.8165) =0.6155 =35.26^\circ\).
\end{solution}
\end{OmitV1}
...
In the example, a \idx{word vector} has five components, \(\sloppy(N_{\text{cat}},N_{\text{dog}},N_{\text{mat}},N_{\text{sat}},N_{\text{scratched}})\) where the various~\(N\) are the counts of each word in any sentence or document. For example,
\begin{enumerate}
...
However, this formula applies only for one-dimensional force and displacement, the case when the force and the displacement are in the same direction.
...
The kinetic energy dissipates when the barbell hits \text{the floor.}
...
\end{tikzpicture}}
\needlines5
\begin{wrapfigure}[5]r{0pt}\mytemp0\end{wrapfigure}
In general, the applied force and the displacement are not in the same direction (as illustrated to the right).
Consider the general case when a vector force~\Fv\ acts on a body which moves a \idx{displacement vector}~\dv.
Then the work done by the force on the body is the length of the displacement times the component of the force in the direction of the displacement---the component of the force at \idx{right-angles} to the displacement does \text{no work.}

\needlines4
\begin{wrapfigure}[4]r{0pt}\mytemp1\end{wrapfigure}
...
%\needlines6
\begin{wrapfigure}[6]r{0pt}\begin{tikzpicture} 
\begin{axis}[footnotesize,font=\footnotesize
  ,axis equal
...
What is the work done by \text{the wind?}
...
A 3\,kg~ball is thrown horizontally from a height of~2\,m and lands~10\,m away on the ground: what is the total work done by gravity on \text{the ball?}
\actposs[4]{\(58.8\) joules}{\(19.6\) joules}{\(29.4\) joules}{\(98\) joules}
\end{activity}
...
\actposs{\((2\uv)\cdot(2\vv)=2(\uv\cdot\vv)\)}
...
The above proof of \cref{thm:dotopsg}, that \(\uv\cdot\uv=0\) if and only if \(\uv=\ov\)\,, may look uncannily familiar.
...
The upcoming \cref{thm:triscal} establishes that this connection between dot products and lengths is \text{no coincidence.}
...
\item 
\begin{figbox}{\vecops13421}%
Here \(|\uv+\vv|=|(5,5)|=\sqrt{5^2+5^2}=\sqrt{50}=7.071\)\,, whereas \(|\uv|+|\vv|=5+\sqrt5=7.236\)\,.
Hence \(|\uv+\vv|=7.071\leq7.236=|\uv|+|\vv|\)\,.
This is called the triangle inequality because the vectors~\uv, \vv, and~\(\uv+\vv\) may be viewed as forming a triangle, as illustrated to the right, and this inequality follows because the length of a side  of a triangle must be less than the sum of the lengths of the other \text{two sides.}
\end{figbox}
\end{enumerate}
\end{solution}
\end{example}
...
The \idx{Cauchy--Schwarz inequality} is one point of distinction between this `vector multiplication' and \idx{scalar multiplication}: for scalars \(|ab|=|a||b|\), whereas the dot product of vectors is typically less, \(|\uv\cdot\vv|\leq|\uv||\vv|\).
...
\end{wrapfigure}
\begin{example} 
The general proof of the \idx{Cauchy--Schwarz inequality} involves a trick, so let's introduce the trick using the vectors of \cref{eg:triscal}.
Let vectors \(\uv=(3,4)\) and \(\vv=(2,1)\).
Then consider the line given parametrically \index{parametric equation}(\cref{def:parlin}) as the \idx{position vector}s \(\xv=\uv+t\vv=(3+2t,4+t)\) for scalar parameter~\(t\)---illustrated to the right.
The \idx{position vector}~\xv\ of any point on the line has length~\(\ell\) (\cref{def:veclen}) where
\begin{eqnarray*}
\ell^2&=&(3+2t)^2+(4+t)^2
...
To prove the Cauchy--Schwarz inequality between vectors~\uv\ and~\vv\ first consider the trivial case when \(\vv=\ov\): then the left-hand side \(|\uv\cdot\vv|=|\uv\cdot\ov|=|\ov|=0\); whereas the right-hand side \(|\uv||\vv|=|\uv||\ov|=|\uv|0=0\); and so the inequality \(|\uv\cdot\vv|\leq|\uv||\vv|\) is satisfied in \text{this case.}
...
We give a special name to vectors \text{at right-angles.}
...
Often the requirement that the orthogonal vectors are nonzero is explicitly made, but beware that sometimes the requirement may be implicit in \text{the problem.}


\begin{example} 
The \idx{standard unit vector}s (\cref{def:stuniv}) are orthogonal to each other.
...
It cannot be done. 
No vector in the plane can be at right-angles to both the standard unit vectors in \text{the plane.}
...
\end{tikzpicture}
\end{wrapfigure}
To introduce the second theorem, imagine trying to draw three unit vectors in any \idx{orientation} in the 2D plane such that all three are at \idx{right-angles} to each other.
The picture to the right illustrates one attempt.
It cannot be done.
There are at most two vectors in 2D that are all at right-angles to \text{each other.}
...
\begin{proof} 
\begin{wrapfigure}r{0pt}
\rotatebox{53.10}{\begin{tikzpicture} 
\begin{axis}[footnotesize,font=\footnotesize
...
\actposs[4]{\(x+3y=10\)}{\(4x+2y=10\)}{\(4x+y=11\)}{\(2x+3y=11\)}
\end{activity}
...
\end{axis}
\end{tikzpicture}}}
\begin{wrapfigure}r{0pt} \mytmp0 \end{wrapfigure}
...
As illustrated in stereo, that means to find all points~\(X\) such that~\(\ovect{PX}\) is orthogonal to~\nv.
...
\needlines6
\begin{wrapfigure}r{0pt}\begin{tikzpicture}
\newcommand{\ppoint}[2]{
...
\end{tikzpicture}
\end{wrapfigure}
The basic idea, as illustrated to the right, is that given any plane (through the origin for the moment), then choosing almost any two vectors in the plane allows us to write all points in the plane as a \idx{sum} of multiples of the two vectors.
With the given vectors~\uv\ and~\vv\ shown to the right, illustrated are the points \(\uv+2\vv\), \(\tfrac12\uv-2\vv\), and \(-2\uv+3\vv\).
Similarly, every point in the plane has a \idx{position vector} in the form \(s\uv+t\vv\) for some scalar parameters~\(s\) and~\(t\).
...
\end{tikzpicture}}}
\needlines{9}
\begin{wrapfigure}[8]r{0pt}\mytemp0\end{wrapfigure}
\begin{example} 
Find a parametric \idx{equation of the plane} that passes through the three points \(P=(-1,2,3)\), \(Q=(2,3,2)\), and \(R=(0,4,5)\), drawn to the right in stereo.
...
A \bfidx{parametric equation} of a plane is \(\xv=\pv+s\uv+t\vv\) where \pv~is the \idx{position vector} of some point in the plane,   the two vectors~\uv\ and~\vv\ are parallel to the plane (\(\uv,\vv\neq\ov\) and are at a nonzero\slash non-\(\pi\) \idx{angle} to each other), and the scalar \bfidx{parameter}s~\(s\) and~\(t\) vary over all real values to give \idx{position vector}s of all points in \text{the plane.}
\end{definition}

The beauty of this definition is that it applies for planes in any number of dimensions.
To do so the \idx{parametric equation}s just use vectors with the corresponding number \text{of components.}
...
\end{wrapfigure}
\begin{example} 
Find a parametric \idx{equation of the plane} that passes through the three points \(P=(6,-4,3)\), \(Q=(-4,-18,7)\), and \(R=(11,3,1)\), drawn to the right in stereo.
...
\begin{wrapfigure}[8]r{0pt} \mytemp0 \end{wrapfigure}
\begin{example} 
Find a parametric \idx{equation of the plane} that passes through the three points \(A=(-1.2,2.4,0.8)\), \(B=(1.6,1.4,2.4)\), and \(C=(0.2,-0.4,-2.5)\), drawn to the right in stereo.
...
\actposs{\((4,1,4)+(3,6,3)s+(2,4,2)t\)}{\(\iv+s\jv+t\kv\)}{\((-1,1,-1)s+(4,2,-1)t\)}{\((3s+2t,4+2s+t,4+3t)\)}
\end{activity}
...
Following \cref{eg:ezyang}, use the \idx{cosine rule} for triangles to find the angle between the following pairs of vectors.  
...
\begin{exercise}  
Recall that \cref{eg:deflsv} represented the following sentences by \idx{word vector}s \(\wv=(N_{\text{cat}}\), \(N_{\text{dog}}\), \(N_{\text{mat}}\), \(N_{\text{sat}}\), \(N_{\text{scratched}})\).
\begin{itemize}
...
\begin{OmitV1}
\begin{exercise}  
Recall \cref{ex:8siambks} found \idx{word vector}s in~\(\RR^5\) for the titles of eight books that The Society of Industrial and Applied Mathematics (\textsc{siam}) reviewed recently.
...
\begin{exercise}  
For each case of \cref{ex:thrpp} that you have done, find two other \idx{parametric equation}s of the plane.
\end{exercise}
...
Why do we not discuss larger angles, such as~\(246^\circ\) or~\(315^\circ\)? nor \text{negative angles?}
