...
half bags, or a third of a cup, which led to the \idx{rational number}s such as~\(\tfrac13\) or \(2\tfrac12=\tfrac52\). 
...
To cope with non-rational numbers such as~\(\sqrt2=1.41421\cdots\) and (pi) \(\pi=3.14159\cdots\), mathematicians define the \bfidx{real numbers} to be all numbers which in principle can be written as a decimal expansion such as~\(\sqrt2\), \(\pi\),
\begin{equation*}
...
He made the first systematic use of negative numbers, published with attribution the solutions of other mathematicians for the cubic and quartic equations, and acknowledged the existence of imaginary numbers. 
 (\mbox{\emph{\idx{Wikipedia} 2015, \cite{Westfall2012}}})}
...
This book mostly uses integers and real numbers, but eventually (\cref{ch:gee}) we need the marvellous \text{complex numbers.}

This book uses the term \bfidx{scalar} to denote a number that could be \idx{integer}, real or complex.
In this book, and before \cref{ch:gee}, a scalar is almost always real valued.
