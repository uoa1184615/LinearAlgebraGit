...
In the mean time people needed to quantify fractions such as two and half a bags, or a third of a cup which led to the {rational numbers} such as \(\tfrac13\) or \(2\tfrac12=\tfrac52\). 
Now \bfidx{rational numbers} are defined as all numbers writeable in the form~\(\tfrac pq\) for integers~\(p\) and~\(q\) (\(q\)~non-zero).
...
Consequently, many areas of modern science and engineering use \bfidx{complex numbers} which are those of the form \(a+b\i\) for real numbers~\(a\) and~\(b\).
