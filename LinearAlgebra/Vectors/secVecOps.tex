...
\item\label[example]{eg:vecadda} 
\((1,3)+(2,-1)=(1+2,3+(-1))=(3,2)\) as illustrated below where (given the two vectors plotted in the margin)
...
\end{center}
\begin{aside}I implement such cross-eyed stereo so that these stereo images are useful both printed and when projected on a screen.\end{aside}
...
In the plane and drawn in the margin are the two \idx{unit vector}s~\iv\ and~\jv\ defined to be of length one and in the direction of the two \idx{coordinate axes}, respectively.
...
Why~\(0.5\)?  Because the \idx{difference} between the vectors \((1.5,3)-(1.2,3)=(0.3,-0.4)\) has length \(\sqrt{0.3^2+(-0.4)^2}=0.5\)\,.
\marginpar{\begin{tikzpicture}
\begin{axis}[footnotesize,font=\footnotesize
...
Conversely, we would like to recognize that vectors \((1.2,3.4)\) and \((3.4,1.2)\) are very different (as also illustrated in the margin)---there is a large `distance' between them.
...
\begin{example} 
Given three vectors \(\av=3\iv+2\jv-2\kv\), \(\bv=5\iv+5\jv+4\kv\) and \(\cv=7\iv-2\jv+5\kv\) (shown below in stereo) use the concept of distance between vectors to answer the following: which pair are the closest to each other? and which pair are furthest from each other?
\begin{center}
...
\ref{thm:vecopsa} is the commutativity of vector addition.
...
\begin{example} 
Which of the following two diagrams best illustrates the \idx{associative law}~\ref{thm:vecopsb}?  Give reasons.
\begin{center}
...
3\xv+(-2\uv+2\uv)&=&6\vv+2\uv
\quad\parbox[t]{10em}{(by \ref{thm:vecopsb}, associativity);}\\
3\xv+\ov&=&6\vv+2\uv
...
\quad\parbox[t]{10em}{(by \ref{thm:vecopse}, distributivity);}\\
...
\quad\parbox[t]{11em}{(by \ref{thm:vecopsd} \& \ref{thm:vecopsa});}
\\3\xv&=&(2\av+\av)+2\xv
\quad\parbox[t]{11em}{(by \ref{thm:vecopsc} \& \ref{thm:vecopsb});}
\\3\xv&=&(2\av+1\av)+2\xv
\quad(\text{by \ref{thm:vecopsh}});
\\3\xv&=&(2+1)\av+2\xv
\quad\parbox[t]{10em}{(by \ref{thm:vecopsf}, distributivity);}
...
\\\xv&=&3\av+\ov
\quad\parbox[t]{11em}{(by \ref{thm:vecopsh} \& \ref{thm:vecopsi});}
\\\xv&=&3\av
\quad(\text{by \ref{thm:vecopsc}}).
\end{eqnarray*}
...
\begin{exercise}  
Prove in detail some algebraic properties chosen from \autoref{thm:vecopsb}--\ref{thm:vecopsj} on vector addition and \idx{scalar multiplication}.
\end{exercise}
...
\begin{exercise}  
For each of the following vector equations, rearrange the equation to get vector~\xv\ in terms of the other vectors.  Give excruciating detail of the justification using \autoref{thm:vecops}.
\begin{enumerate}
\item \(\xv+\av=\ov\).
\item \(2\xv-\bv=3\bv\).
