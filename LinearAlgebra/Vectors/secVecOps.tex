...
In the plane and drawn in the margin are the two \idx{unit vector}s~\iv\ and~\jv\ (length one) in the direction of the two \idx{coordinate axes}.
...
Similarly, in three dimensional space we often write vectors in terms of the three vectors~\iv, \jv\ and~\kv, each of \idx{length} one, aligned along the three \idx{coordinate axes}.
For example,
\begin{eqnarray*}
...
\begin{exercise} \label{ex:} 
Find a \idx{parametric equation} of the line through the given two points.
\begin{parts}
\item \((-11,0,3)\),
   \((-3,-2,2)\)
