...
\item The addition \((1,3)+(3,2,0)\) is not defined and cannot be done as the two vectors have a different number of components, they have different sizes.
\end{enumerate}
\end{example}
...
Because the \idx{difference} between the vectors \((1.5,3)-(1.2,3)=(0.3,-0.4)\) has length \(\sqrt{0.3^2+(-0.4)^2}=0.5\)\,.
\marginpar{\begin{tikzpicture}
\begin{axis}[footnotesize,font=\footnotesize
...
Moreover, the proofs show the vector properties follow directly from the familiar properties of arithmetic operations \text{on scalars.}
